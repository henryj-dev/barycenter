...
잡이 **`deploy/` · `tests/install/` · 그 워크플로 자신이 바뀐 PR 에서만** 돈다. 모든 PR 에
안 거는 이유는 다섯 배포판 컨테이너가 임계 경로를 크게 늘리기 때문이고(#15 가
873초→254초로 줄인 직후다), nightly 로 안 보내는 이유는 그러면 *"머지 뒤에 안다"* 가
되기 때문이다. **바뀐 것이 그것일 때 머지 앞에서 잰다.**
