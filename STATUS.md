...
CLI 는 listener·풀·HTTP proxy 라우트 create 가 있다. apply 는 아니다.
라우트 websocket 은 기본이 끈다. redirect·reject 는 아직이다.
...
| CLI | `changeset` 단계, `commit --plan`, `apply --plan`, export/import, status/rollback, listener create, 풀 create, HTTP proxy 라우트 create | redirect·reject·패스스루 라우트 |
...
- CLI 리소스 하위 명령 (redirect·reject·패스스루 라우트)
