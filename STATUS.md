...
읽기를 연다. 모르는 이름은 안 부른다. apply 는 아니다. websocket 은 HTTP proxy
에서만 켠다. 인증서 패치에 개인키가 없다.
...
| CLI | `changeset` 단계, `commit --plan`, `apply --plan`, export/import, status/rollback, listener·풀·라우트·백엔드·TLS 정책·인증서·SNI create, listener·라우트·백엔드·SNI delete, get(인증서·정책·SNI·헬스) | 드레인 명령 (S2) |
