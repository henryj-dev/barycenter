...
**셋을 쌓았으므로 `--freeze` 가 열린다.** 선언은 사람이 지는 결정이라 아직 안 했다 —
선언하는 날 `--freeze-gate` 를 어디에 걸지(게이트에 넣을지, nightly 로 보낼지)가 함께
정해져야 한다(셋째 회차 C).

⚠️ **설치 하네스는 게이트에도 CI 에도 없다** (둘째 회차 D). `tests/install/run.sh` 는
다섯 배포판 실물 컨테이너를 도는데 `verify.sh` 도 워크플로도 그것을 안 부른다 —
`package.json` 의 `test:install` 뿐이다. **어디에 넣을지가 결정 사항**이라 이 회차에서는
안 넣었다: 다섯 컨테이너는 임계 경로를 크게 늘리고(#15 가 873초→254초로 줄인 직후다),
nightly 로 보내면 "머지 뒤에 안다" 가 된다.
