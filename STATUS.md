...
CLI 는 listener·풀·HTTP proxy·redirect 라우트 create 가 있다. apply 는 아니다.
redirect 는 to·status. 기본 302. pool 이 없다. reject 는 아직이다.
...
| CLI | `changeset` 단계, `commit --plan`, `apply --plan`, export/import, status/rollback, listener create, 풀 create, HTTP proxy·redirect 라우트 create | reject·패스스루 라우트 |
