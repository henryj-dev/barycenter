...
CLI 는 listener·풀·HTTP proxy·redirect·reject 라우트 create 가 있다. apply 는 아니다.
reject 는 403·404·444. 기본 403. to·pool 이 없다. 패스스루 라우트는 아직이다.
...
| CLI | `changeset` 단계, `commit --plan`, `apply --plan`, export/import, status/rollback, listener create, 풀 create, HTTP proxy·redirect·reject 라우트 create | 패스스루 라우트 |
