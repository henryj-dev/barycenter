...
| 단위 | `npm test` | **812** |
| conformance | `npm run test:conformance` | **449** |
...
| §11.3 동적 포트 | k8s 네이티브 배포는 여전히 별도 과제다 — 설계의 결정이고 안 뒤집는다. 다만 어느 배포를 고르든 필요한 **소켓 집합**을 낸다: `GET /api/v1/sockets` · `bary get sockets` |
...
⚠️ **S12 가 게이트 부하 아래에서 간헐로 떨어진다** (2026-08-24). 단독으로는 다섯 번
연속 초록이다. 서명 하나를 붙잡았다 — 복구 러너가 `DpRejection(terminalState:'failed')`
로 죽었고, 그 회차에도 `link`·`served` 는 둘 다 맞았다(**세상은 맞고 좌표만 뒤에 남는다**).
가설이지 확정이 아니다. 게이트 안의 흔들림은 빨강을 무시하는 법을 가르치므로 **다음
회차의 첫 일**이다.
