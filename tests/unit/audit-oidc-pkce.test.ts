 ...
 * ── 왜 검증자를 클라이언트가 들고 있나
 *
 * 이 RP 는 **상태를 안 갖는다.** `state` 도 이미 그렇게 다룬다 — 서버가 만들어 주고
 * 브라우저의 `sessionStorage` 가 보관했다가 되보낸다. 노드가 여럿일 때 시작과 교환이
 * 다른 노드에 닿을 수 있으므로 서버 메모리에 두면 그 순간 깨진다.
 *
 * 그래서 `code_verifier` 도 같은 자리에 산다. **이건 SPA 의 표준 형태다.** 다만 nonce 가
 * 이 구조에서 무엇을 사고 무엇을 못 사는지는 분명히 해 둔다: 클라이언트가 되보내는 값
 * 이므로 클라이언트를 장악한 자에게는 안 통한다. 훔친 `id_token` 주입을 막는 것이지
 * 장악된 브라우저를 막는 것이 아니다. 가로챈 `code` 를 막는 것은 **PKCE** 쪽이다.
 */
