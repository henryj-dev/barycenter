 ...
 *
 * **이 테스트는 증거 쓰기 자리를 못 밟는다** (37차 실측 — id-only 로 되돌려도 초록).
 * 게이트를 **첫 번째** 관측(`reload_intent` 진입)에 걸었기 때문이다: 그러면 낡은 러너는
 * 깨어난 뒤 phase 전이 쓰기에서 `stale_leader` 로 죽어 증거 쓰기에 못 간다.
 *
 * **게이트를 한 칸 옮기면 된다** — `awaitActivation` 안의 **두 번째** 관측. 그러면
 * 낡은 러너가 HUP 까지 끝낸 뒤 정지하므로 깨어나면 곧장 증거 쓰기다. 그 재현이
 * `debt-evidence-pollution.test.ts` 에 있다. **"못 만들었다" 는 불가능이 아니라
 * 내 조립이 틀린 것이었다.**
 *
 * **"agent 층 fence 를 지킨다" 도 후했다** (38차 실측). `writeJournal` 의
 * `assertLeader` 를 지워도 이 파일 넷이 전부 초록이다 — 겹치는 층들(seq CAS · 홀더
 * 검사 · `assertOwnership`)이 최종 상태를 같게 만들기 때문이다.
 *
 * **그래서 검출력이 없다.** 지우지 않는 이유는 오직 하나 — 이 무대(관측 게이트 + 승계)가
 * 다른 재현의 발판이라 남겨 두면 다음 사람이 다시 안 짜도 된다. **자산이 아니라
 * 발판이고, 그렇게 적는다.** 두 번 격하했다: "증거 오염을 지킨다" → "agent 층 fence 를
 * 지킨다" → **"아무것도 안 지킨다".**
 */
it('낡은 러너가 fence 를 산 채로 건너도 agent 층이 막는다 (증거 쓰기는 안 밟는다)', async () => {
  const store = new MemoryStore();
  const zero = { epoch: '0', rev: '0' };
  const one = { epoch: '1', rev: '1' };
