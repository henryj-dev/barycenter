...
it('재현 B — 같은-id 비종단 고아 저널 위의 재발급은 자기 청소에 걸린다', async () => {
  ...
  expect(
    ...
    // **48차에 기대값이 바뀌었다 — 계약이 바뀌었기 때문이다.**
    //
    // 34차에 이 자리를 고칠 때는 `tuple_mismatch`(정직한 거부)가 옳은 답이었다. 그때는
    // 이름 원장이 **이름 단위**였고, 같은 이름의 재발급은 옛 기록에 걸리는 것이 정상이었다.
    //
    // 45차에 계약을 적었고(**포기는 전환에 붙지 이름에 붙지 않는다**) 48차에 멱등 캐시의
    // 키에도 토큰을 넣었다. 이제 신임의 재발급은 **자기 칸**을 받아 통과한다 — 그것이
    // 계약이 약속한 것이다. 옛 전환은 자기 칸에서 `aborted` 로 닫혀 있다(`ledger`).
    //
    // **테스트가 재던 것이 계약 이전 의미론이었다.** 계약을 바꾸면 그 계약을 핀하던
    // 테스트도 같이 바뀌어야 하고, 안 바꾸면 둘 중 하나가 거짓말한다.
    first: 'ok',
    second: 'ok',
    ledger: 'aborted',
    cacheAlive: true,
