...
describe('신원 비교 자리는 세어져 있다', () => {
   ...
   * 리뷰에서 보인다.**
   */
  // **42차에 22 → 20.** 슬롯 부류(`releaseStaleSlots`·`supersede`·`releaseHolderSlots`)를
  // `ownsSlotOp` 하나로 모아 직접 비교 셋이 사라졌다. **줄어드는 방향도 이 테스트가
  // 잡는다** — 무엇이 사라졌는지 확인하고 내리라는 것이 설계 의도다.
  // **43차에 20 → 20.** `sameTransitionName` 을 신설해 `supersede` 의 `sameOp` 까지
  // 그것으로 모았다 — 직접 비교가 늘었다 줄었다 하는 것을 계측기가 매번 잡았다.
  // `sameTransitionName` 을 신설했다 — "이름을 묻는 자리" 와
  // "그 전환인지 묻는 자리" 를 술어로 갈랐다. 직접 비교가 하나 늘었지만 그것이
  // **이름을 묻는 유일한 자리**이고, "왜 토큰을 안 보나" 라는 물음이 자리마다가 아니라
  // 그 함수 하나에 붙는다. 부류 마감 게이트가 요구하는 모양이다.
  // **검수 2026-08-24 (G5) 에 20 → 19.** `FakeEffects` 가 `src/dp/apply.ts` 에서
  // `src/testing/apply-fakes.ts` 로 나갔고, 그 안의 직접 비교 하나가 함께 나갔다.
  // **줄어든 이유가 「자리가 사라졌다」가 아니라 「프로덕션이 아니게 됐다」다** — 이
  // 계측기가 세는 것은 프로덕션 코드의 신원 비교이고, 테스트용 가짜는 그 물음의
  // 대상이 아니다(`reachable.mjs` 도 같은 경계를 쓴다). 그 자리는 여전히 있고,
  // 여전히 id 만 보며, 여전히 옳다 — 가짜의 판정은 실물 계약이 아니기 때문이다.
  it('직접 비교 자리가 19 곳이다', () => {
    const found = sites();
    expect(
      found.length,
