...
describe('passive 를 안 넣은 이유 (§4.3 · S15.5)', () => {
   ...
   */
  it('밸런서가 만지는 dict 키는 넷뿐이다 — 모르는 키가 생기면 걸린다', () => {
     ...
     * 그것을 읽기만 한다. `in:`·`rr:` 은 고르기 위한 보조값이다.
     *
     * **`attr:` 가 넷째로 들어왔다** (2026-08-31, ADR 3 조각 2). 이 목록을 늘리는 것은
     * 가볍지 않으므로 근거를 남긴다 — 아래 검사가 그 근거를 계약으로 만든다.
     */
    expect([...counters].sort(), '모르는 dict 키가 생겼다').toEqual(['attr', 'in', 'rr', 'slot']);
  });

  /**
   * **이것이 원래 계약의 진짜 뜻이다.**
   *
   * 옛 검사는 키 **개수**를 셌고, 근거는 *"네 번째가 생기는 순간 「이 백엔드가 풀에
   * 있는가」 의 주인이 둘이 될 수 있어서"* 였다. 그런데 위험한 것은 **개수가 아니라
   * 쓰기**다 — `passive` 가 안 되는 이유는 그것이 **데이터 경로가 스스로 쓰는** 카운터라
   * 능동 프로버(`backend_health` → `projectHealth`)와 주인이 둘이 되기 때문이다.
   *
   * `attr:` 에는 그 성질이 없다. 쓰는 쪽이 `slot:` 과 **같은 권한**이다 — 커밋된 모델에서
   * 컨트롤 플레인이 admin 창구로 민다. 밸런서는 읽기만 한다.
   *
   * 그래서 못 박는 것을 개수에서 **쓰기 금지**로 옮긴다. 누가 `attr:` 에 쓰기 시작하면
   * 그 순간 주인이 둘이 되고, 그때 이 검사가 빨개진다.
   */
  it('**밸런서는 `attr:` 를 안 쓴다** — 쓰기 시작하면 주인이 둘이 된다', () => {
    const conf = render(model, ON).conf;
    const writes = conf.match(/d:(?:set|incr)\("attr:/g) ?? [];
    expect(writes, `밸런서가 attr: 에 썼다: ${JSON.stringify(writes)}`).toEqual([]);
    // 읽기는 있어야 한다 — 없으면 이 검사가 아무것도 안 지킨다.
    expect(conf).toMatch(/d:get\("attr:/);
  });
});
