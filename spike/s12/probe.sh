...
while [ "$i" -lt "$TOTAL" ]; do
  ...
  # ── 잡아 둔 서명 하나 (2026-08-24) ─────────────────────────────────────
  #
  # 이 스파이크는 **게이트 부하 아래에서만** 간헐로 떨어진다. 단독으로는 다섯 번
  # 연속 초록인데 전체 게이트 안에서는 회차마다 **다른 지점**이 빨개진다
  # (#28 → #25 → 없음). 위 판별법대로면 고정된 로직 결함이 아니다.
  #
  # 실패한 회차의 진단을 처음으로 붙잡았다. 복구 러너가 이렇게 죽어 있었다:
  #
  #     terminalState: 'failed'
  #
  # 즉 `DpRejection('terminal')` 이다 — **앞선 복구 시도가 활성화 증거를 예산 안에
  # 못 보고 `fail` 을 찍었고**, 그 뒤 재시도가 원장에 막힌 것이다. 그러면 아래
  # 재시도 루프는 **영영 못 지난다**: 같은 전환은 이미 종단이다.
  #
  # 그리고 그 회차에도 `link` 와 `served` 는 둘 다 gen-2 였다. **세상은 맞았고 좌표만
  # 뒤에 남았다** — 이 파일이 next 단계에 대해 이미 적어 둔 S7 의 그 모양이다.
  # 다만 좌표가 안 옮겨졌으므로 gen-3 의 `expectedCurrent` 가 안 맞고, 그래서 next 도
  ...
  if [ "$REC_PHASE" = "activated" ] && [ "$LINK" = "generations/gen-2" ] \
    ...
    # **꼬리 4 줄로는 모자랐다.** 실측에서 남은 것이 `terminalState: 'failed'` 와
    # `Node.js v24.18.1` 뿐이었다 — 예외의 **종류와 메시지**가 그 앞에 있는데 잘렸다.
    # 진단이 원인을 못 담으면 진단이 아니다.
    echo "        복구 로그:"; echo "$REC_OUT" | tail -25 | sed 's/^/          /'
    echo "        후속 로그:"; echo "$NEXT_OUT" | tail -25 | sed 's/^/          /'
  fi
  i=$((i+1))
done
