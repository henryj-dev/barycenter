...
1. **외부 주소를 지정했을 때 평문 제어 API를 허용하는 것** — 기본 바인드는 루프백이지만,
   외부 주소를 지정하면 TLS 없이도 경고만 남기고 기동한다.
2. **검사가 조용히 건너뛰어지는 것** — 일반 `FsEffects` 주입 계약에는 예외 fail-open 경로가
   있고, 운영 부트스트랩에는 빈 명령을 성공으로 처리하는 경로가 있다.
3. **입력·인증 경계의 좁은 구멍** — manifest의 중복 리소스 식별자, 제어 API의 레이트리밋 부재.

## 발견 사항

### [높음] 외부 바인딩에서 평문 제어 API를 허용함 (조건부)

파일: `src/bin/barycenterd.ts:743`, `src/bin/barycenterd.ts:772`

기본 `BARY_LISTEN`은 `127.0.0.1:8088`이라 기본 설정은 루프백이다. 다만 TLS가 꺼진 상태에서
운영자가 루프백 외 주소를 지정하면 API를 바인딩할 수 있으며, 현재는
`log.warn('listen.exposed')` 경고만 남긴다. 이 API에는 Bearer 토큰, 설정 변경·적용 권한,
인증서 개인키 업로드가 존재한다.
...
붙는다. 문제는 "TLS가 없다"가 아니라 **외부 주소를 선택한 배포에서 안전하지 않은 조합을
허용하고 경고로 끝낸다**는 것이다. 실제 위험도는 네트워크 경로와 앞단 TLS 종단 여부에 좌우된다.
...
### [중간] 설정 검사 계약에 예외 fail-open 경로가 있고, 빈 명령은 성공으로 처리됨

파일: `src/dp/effects-fs.ts:188`, `src/dp/effects-fs.ts:194-197`, `src/dp/operation.ts:131`

두 갈래가 있고, **1차 보고서는 첫 번째만 봤다.** 다만 일반 `FsEffects` 주입 계약과 실제
운영 부트스트랩 경로를 구분해야 한다.

1. `BARY_CONFIGTEST_CMD`가 없으면 manifest·digest 대조만 하고 넘어간다 (`:188`).
2. 일반 `FsEffects`에 주입된 `configTest`가 던지면 `{ ok: true }`를 돌려준다 (`:194-197`).
...
다만 실제 운영 경로인 `src/dp/effects-boot.ts:107-114`의 `configTest` 래퍼는
`execFile('/bin/sh', ...)` 실패를 `false`로 변환한다. 따라서 nginx 바이너리 경로가 바뀌거나
컨테이너에서 exec가 실패하는 경우에는 운영 경로에서 preflight가 거부되며, 보고서의
"실행 실패가 그대로 `{ok:true}`가 된다"는 서술은 일반 주입 구현에만 해당한다.

운영 경로에는 별도의 빈 값 문제가 있다. `BARY_CONFIGTEST_CMD`가 **정의되어 있지만 빈 문자열**이면
`/bin/sh -c ''`가 성공하므로 설정 검사가 사실상 비활성화된다. 미설정과 빈 문자열을 구분하지
않고 성공으로 처리하는 것은 운영 구성 오류를 숨긴다.
...
- 호출자가 검사 실행을 위임했지만 예외를 던지는 구현에서는 잘못된 설정이 게시된 뒤 reload
  단계에서야 실패할 수 있음

운영 부트스트랩의 빈 명령 영향:

- 검사를 설정했다고 생각한 배포가 실제로는 빈 셸 명령을 성공한 것으로 처리함
- 적용 실패가 늦게 드러나고, 새 세대와 현재 포인터 상태가 운영자 관점에서 혼란스러워짐

권장 조치:

- 일반 `configTest` 주입 계약에서는 실행 예외를 `ok: false`와 별도 오류 상태로 승격
- `effects-boot.ts`에서는 현재처럼 실행 실패를 `false`로 유지하되, `BARY_CONFIGTEST_CMD`가
  빈 문자열이면 기동 시 거부
...
정한다. 따라서 이건 인증 로직의 결함이 아니라 **경계의 부재**다. 기본 루프백 바인드와
고엔트로피 토큰을 전제로 하면 즉시 악용되는 결함이라기보다, 외부 노출·평문 전송과 결합할 때
위험도가 커지는 방어 심층화 공백이다.
...
넘으면 연결을 닫고 클라이언트가 전체 스냅샷으로 재연결하게 한다)의 결함이 아니다. 현재 GUI는
`Last-Event-ID` 기반 이벤트 재생을 사용하지 않는다.
...
2. `BARY_CONFIGTEST_CMD` 빈 문자열을 기동 시 거부하고, 일반 `configTest` 예외는 실패로 기록하기
...
| [중간] nginx 설정 검사가 선택 사항 | **[중간]으로 조정·범위 수정** — 실제 운영 래퍼의 실행 실패는 `false`이며, 일반 주입 예외와 빈 명령 문제를 구분 |
