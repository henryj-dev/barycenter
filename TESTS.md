...
> **왜 일부만 RUNNABLE 인가.** 처음엔 엔진이 정해 준 순수 함수만 돌렸다. 지금은
> changeset · apply · GUI 패치 · ACME 원장도 돈다. 표에 **RUNNABLE** 이 없는 행은
> 여전히 명세다 — 스텁을 만들지 않는다. 동결(§9.1.1)과 실행은 다르다.

## 검수 반례를 다루는 순서

열한 번의 검수를 지나며 **같은 실수를 세 라운드 연속** 했다. 반례를 고치고 나서 테스트를
썼고, 그러면 **이미 통과하는 테스트**를 쓰게 된다. 뮤테이션을 돌려야 비로소 그게 드러났다.

10차 검수는 더 나쁜 것도 찾았다 — 수렴 방식으로 바꾸면서 `review7` 픽스처를 **코드에
맞춰 고쳐** 놓고 통과시켰다. 커밋 diff 가 증거였다.

원인은 습관이 아니라 **순서**다. 그래서 순서를 적어 둔다.

### 1. 반례를 먼저 재현한다 — red 를 눈으로 본다

검수가 말한 것을 그대로 실행해 **실패하는 것을 확인한다.** 통과하면 반례를 잘못 옮긴
것이지 코드가 멀쩡한 것이 아니다. 실제로 여러 번 그랬다.

임시 프로브(`tests/unit/_probeN.test.ts`)로 확인하고, 확인이 끝나면 지운다.
남길 것은 conformance 다.

### 2. conformance 로 옮긴다 — 여전히 red 여야 한다

`tests/conformance/reviewN-*.test.ts` 에 적는다. 이때도 **실패해야 한다.**
여기서 통과하면 1번의 재현과 다른 것을 쓴 것이다.

### 3. 그다음에 고친다

### 4. 뮤테이션으로 변별력을 확인한다

고친 곳을 **일부러 되돌려** 테스트가 잡는지 본다. 안 잡히면 **테스트가 잘못된 것이다.**

지금까지 안 잡힌 이유들:
- 제목이 검증 범위보다 넓었다 (`publishCalls === 0` 을 안 보면서 "부작용 전에 막힌다")
- 창을 못 맞췄다 (승계가 이미 끝난 뒤에 시험 → 아무것도 검증 안 됨)
- 이름만 봤다 (주인 이름은 그대로 두고 내용만 덮을 수 있다)
- 모의로만 봤다 (`FakeEffects` 로 lease 를 시험 — 파일을 만지는 건 `FsEffects` 다)
- 뮤턴트를 잘못 만들었다 (검사를 지웠더니 그 아래가 TypeError → 잡힌 것처럼 보임)

### 5. 못 고친 것은 못 고쳤다고 적는다

`scripts/verify.sh` 의 게이트와 `STATUS.md` 에. 회차별 ▲/❌ 표는
`docs/archive/STATUS-log.md` 다. ▲ 는 부분, ❌ 는 미해소다.
...
**열 회차 반복된 모양이다.** 매번 이렇게 됐다.

1. 반례를 고치려고 "이 상태에서는 그 답을 믿을 수 없다" 는 규칙을 하나 만든다
2. 그 검사를 **한 자리**에 넣는다 (대개 반례가 났던 자리)
3. 다음 검수가 **다른 자리**에서 같은 병을 찾는다

14차에 "읽고-확인" 집합을 만들었는데 17차가 거기에 후보를 안 넣었고, 18차가 그 구멍을
찾았다. 13차 ①④ · 16차 · 17차 A · 18차 #1 이 전부 같은 모양이다.

**"잊지 말자" 는 계약이 아니다.** 두 가지로 막는다.

- **표를 요구한다.** `converged`·`repaired` 는 `Settled` 표 없이 만들 수 없고, 표는
  `#settled` 만이 만든다. 새로 봐야 할 것이 생기면 거기 한 곳에만 넣는다. lease 를
  `Checked` 로 강제한 것과 같은 수법이다 — **타입이 의무를 지면 훑어 다니지 않아도 된다.**
- **반환값을 속성으로 본다.** 모델의 P7 이 그것이다. 수렴의 거짓 답 넷이 전부 사람
  손에 잡혔던 이유는 **반환값을 보는 속성이 하나도 없어서**였다.

새 규칙을 넣을 때 스스로 물어라: **이 규칙을 어기는 답이 나올 수 있는 자리가 몇 개인가.
그 자리를 전부 지나는 좁은 문이 있는가.**

### 교차가 얽힌 반례는 모델에 넣는다

`tests/model/` 이 스케줄을 **생성한다.** 손으로 짠 반례는 내가 상상한 교차만 본다 —
열네 라운드 동안 그랬고, 13차 ①④ 를 재현 못 한 것도 그래서였다.

```bash
npm run test:model
```

리더 교체·외부 효과·종단 재진입·여러 인스턴스가 얽힌 반례라면 **먼저 모델에 시나리오를
넣어 본다.** 스케줄러가 그 부류를 더 찾아 주는 일이 실제로 있었다(반례 ③ 이 그렇게 나왔다).

**모델이 못 잡으면 이유가 둘 중 하나다.**

- **무대가 넓다** — 교차가 길어 표본에 안 들어온다. 준비 단계로 무대를 좁힌다. 교차를
  고르는 것이 아니다: 누가 언제 끼어드는지는 여전히 스케줄러가 정한다.
- **속성이 없다** — 볼 수 있는 자리가 아니다. 상태에 안 남는 것(게시·HUP 같은 외부 효과)은
  `World` 가 적고 속성이 본다. P6 이 그렇게 생겼다.

둘은 고치는 방법이 다르므로 **먼저 갈라야 한다.** 뮤테이션이 안 잡히면 그 자리를
*지나기만* 한 것인지, 지나는데 *볼 수가 없는* 것인지 확인한다.

**모델이 초록이라고 안전한 게 아니다.** 매번 "상한에서 끊었다" 가 찍히고, 그건 "여기까지는
못 깼다" 는 뜻이다. 그리고 **모델도 뮤테이션을 돌려야 한다** — 한 번은 `allSettled` 가
`InvariantViolation` 을 삼켜서 일부러 넣은 버그에도 초록이었다.

시나리오를 넣을 때는 **무엇을 못 보는지** 같이 적는다. 두 평면이 늘 성공하던 동안 모델은
부분 커밋 축을 통째로 못 봤고, 그 사실이 파일에 적혀 있어서 메울 수 있었다.

### 뮤테이션은 손으로 고르지 않는다

반례를 고칠 때 그 자리에 뮤테이션을 넣는 것은 계속 한다 — 그건 "이 수정이 값을 하는가"
를 재는 것이다. 그런데 그것만 하면 **내가 의심한 곳만** 검사한다.

```bash
npm run mutate -- --limit 120          # 규칙으로 생성해 훑는다 (기본은 80)
npm run mutate -- --file src/dp/driver.ts --limit 40
```

`src/dp` 다섯 파일에서 **519 개**가 나온다(23차 시점). 시드가 고정돼 일부만 돌려도
재현된다. **이 숫자는 코드가 자라면 같이 자란다** — 23차가 "문서에 439 라고 적혀 있는데
실제로는 515 다" 라고 짚었다. 계측기 눈금이 문서와 어긋나면 점수도 같이 못 믿는다.
**살아남은 뮤턴트가 검사되지 않은 행동이고 다음 버그가 날 자리다.**

## 근거를 쓸 때 그것이 참인지 재는 법 (39차)

34~38차에서 **매 회차 새로 쓴 근거가 다음 회차에 거짓으로 판명됐다.**

```
35차 "동치"                → 36차가 뒤집음
37차 "동치" · "구조적 무해"  → 38차가 뒤집음
```

지금까지의 처방은 전부 **"다음 회차가 잡는다"** 였다. **그건 처방이 아니라 관측이다.**

거짓으로 판명된 근거들의 공통형은 **양상 어휘("동치·불능·구조적·도달 불가")를 측정
없이 쓴 것**이고, 참으로 남은 근거들은 전부 측정 첨부형("재 봤다·확인했다")이었다.
그래서 **근거를 유형으로 나누고 유형마다 검증물을 요구한다.**

| 근거 유형 | 쓰는 말 | **요구되는 검증물** |
|---|---|---|
| **위임형** | "하류/상류 가드가 막는다" | 그 가드를 지우는 뮤턴트가 **빨간** 테스트를 짝으로 |
| **불변식 함의형** | "I⟨n⟩ 이 보장하므로 불능" | 그 주장을 **trial invariant 로 걸고** 스위트를 한 번 돌린 결과 |
| **자가치유형** | "수렴이 덮는다" | 치유 경로의 **결정적 재현 하나** |

선례가 전부 레포 안에 있다. I4 를 "넣어 보고 알았다 — 86 개가 터진다" 로 기각한 것이
둘째 유형의 실물이고, 38차가 `finishOperation` 토큰 검사를 지워 위임이 살아 있음을
확인한 것이 첫째의 실물이다. **37차는 그 방법을 알면서 자기 I1 주장에 안 썼다.**

**그리고 그 규칙을 게이트로 옮겼다** (48차 처방 B) — `scripts/pinned.mjs`.
`src/` 를 바꾼 커밋은 커밋 메시지에 `Pinned-by:` 를 적어야 하고, 게이트가 **그 테스트를
부모 트리(수정 전)에 대고 돌려 빨간지 확인**한다. 셋을 잡는다:

```
표식 없음        src 를 바꿨는데 Pinned-by 가 없다
거짓 표식        고른 테스트가 0 건이다 (오타·없는 파일)
안 지키는 핀     수정 전에도 초록이다 — 아무것도 안 지킨다
```

**왜 게이트여야 했나**: 39차에 규칙을 세우고 40차에 강화했는데 **46차에 또 어겼다.**
그리고 45차의 계약 핀은 **재발급을 실행하지 않는 픽스처 통과**였다(48차 CE-48-A).
**규칙이 산문이라 안 지켜진다.**

**게이트 자신이 지금까지 아홉 번 틀렸다** — 아홉째는 **커밋 하나만 보고 있었다**는 것이다.
`base..head` 를 통째로 diff 하면서 메시지는 HEAD 것만 읽었으니, 커밋 셋을 밀면서 가운데만
재현물 없이 `src/` 를 고치고 마지막에 핀이 있으면 **그냥 통과**했다. 실측으로 확인했다 —
같은 범위에서 옛 게이트는 0, 새 게이트는 1 과 함께 그 커밋의 sha 를 짚는다. CI 에 걸면서
드러났다: **범위를 받는 도구는 범위를 봐야 한다.** — 여덟째는 **새 테스트 디렉토리를 만든 커밋을
"고른 테스트가 0 건" 으로 오차단**한 것이다. `tests/` 만 얹고 러너 설정은 부모 것을 썼으니
부모의 `vitest.config.ts` 가 그 디렉토리를 몰라 파일을 못 찾았다. 게이트의 계약은 *"지금의
테스트를 부모의 `src/` 에 대고 돌린다"* 이므로 **러너 설정은 재는 도구 쪽**이다 — 함께 얹도록
고쳤다. 대상(`src/`)은 그대로 부모 것이다. — 만들면서 둘(인자 쪼개기 · 없는 파일을
"빨갛다" 로 셈), 49차가 넷(본체 오염 · 작업물 파괴 · 신규 파일 오판 · 근거 없는 `none`),
그리고 **worktree 안에서 `HEAD` 가 부모를 가리키는 것**을 못 봐서 하나. 마지막 것은
"지금 테스트를 얹는다" 가 **부모 테스트를 도로 얹는** 것이 되어 정당한 핀을 오차단했다.

**계측기를 만들 때마다 계측기 자신에게 결함이 있었고**(스윕·P8·P10·P11·census 에 이어
일곱 번째), 이 게이트는 **한 계측기 안에서만 일곱 번**이다. 그래도 두는 이유는 그것이
잡는 것(재현물 없는 커밋 · 픽스처 통과)이 **사람이 반복해서 놓친 것**이기 때문이다.

원래 적었던 것: — 인자를 통째로 쪼개 vitest 가 아무것도 안 고르고
초록이었고, 없는 파일이 던지는 것을 "빨갛다" 로 세어 거짓 표식이 통과했다. 손으로 돌려
보고 알았다. **계측기를 만들 때마다 계측기 자신에게 결함이 있었다**(스윕·P8·P10·P11·
census — 여섯 번째다).

**이 게이트가 못 잡는 것**: 산문의 정확성("이주는 불가능하다" 같은 전칭 서술).
그건 원래 검수로만 잡히는 축이고 카운터와 분리한 현행 구조가 맞다 — 48차 판정이다.

**검증물은 커밋된 산출물이어야 한다** (40차). "재 봤다" 는 문장 하나로 충족되는 규칙은
이 레포가 여섯 번 앓은 병의 입구를 다시 연다 — 39차가 규칙을 세운 그 커밋에서 "재현으로
확인했다" 를 재현물 없이 두 번 적었다. 뮤턴트든 재현 테스트든 **레포에 들어와 있어야**
검증물이다.

**검증물 없는 근거는 "불가지" 로만 적는다.** "동치다" 가 아니라 "가르는 무대를 못 찾았다"
이고, 그 둘의 차이가 이 레포가 여섯 번 걸린 함정이다.

### census 는 손으로 세지 않는다

신원 비교 자리를 **여섯 번 손으로 세고 여섯 번 다 뭔가 나왔다.** 그래서 기계화했다 —
`tests/conformance/census-identity.test.ts`.

> ⚠️ **여기 있던 문장이 거짓이었다** (40차가 판정, 41차가 삭제). 원래는 *"만들자마자
> 아무도 안 센 `assertInvariants` 의 토큰 대조가 나왔다"* 였는데, 계측기가 클래스
> 메서드를 못 봐서 `finishOperation` 의 자리(10차부터 세어져 있던 것)를 **오귀속**한
> 것이었다. 나는 그 잘못된 이름표에 맞는 이야기를 지어 냈다.
>
> 더 나쁜 것은 **40차가 그것을 "전부 거짓" 으로 판정하고 코드 주석은 고치면서 이
> 문단은 안 고쳤다**는 점이다. 그 커밋에 *"이름을 붙이는 것과 전 자리를 고치는 것은
> 다른 일이다"* 라고 적었는데, **같은 파일 8 줄 아래에 새 규칙을 추가하면서 이것을
> 지나쳤다.** 41차가 짚었다.

그 테스트가 잡는 것은 **"이 자리가 옳은가" 가 아니라 "이 자리를 누가 셌는가"** 다.
판정은 사람이 하고, 계측기는 **판정 없이 지나가는 것**만 막는다.

**실제로 그 일이 일어났다** (41차): CE-41-A 를 고치며 `ownsSlotOp` 을 넣자 이 테스트가
즉시 빨개졌다. 새 자리가 판정 없이 지나가지 않았다.

**계측기의 한계도 실측됐다.** ⚠️ **여기 "검출은 모든 모양에서 살아 있다" 고 적었는데
과장이었다**(42차가 실측으로 뒤집었다). **집합이 안 변하는 모양에서는 검출이 0 이다** —
기존 owner 함수 **안에** 새 대조를 추가하거나(E7), `normalizeNumeric` 없는 **원시 토큰
비교**(E8)면 둘 다 초록이다. 41차 보고에 그 둘이 GREEN 으로 적혀 있었는데 내가 옮겨
적으면서 넓혔다 — **계측기 한계의 과대 서술이 미래 서사의 원료가 되는** 39차 병형이다.

정확히는: 검출은 **집합이 변할 때만** 산다. 그리고 **귀속(이름표)은 세 모양에서
거짓말한다** — 모듈 `const` 화살표 · getter · 최상위 함수
본문의 2 칸 들여쓴 줄 뒤. 39차의 병("이름표에 맞는 서사를 지어냄")의 **무대가 그대로
남아 있다.** 이름표를 서사의 원료로 쓰지 마라.

---

점수 이력 — 시드가 다르면 표본이 달라지므로 회차 간 비교는 대략치다.

```
13차 첫 측정  76.4%  →  반례 채운 뒤 80.0%  →  80.7%  →  22차 75.7%  →  23차 86.4%
24차 81.8%  →  27차 76.4%  →  33차 84.5% (26차 계약 이동 이후 첫 측정)
→  **35차 83.6%** (가지치기 이후 첫 측정 · 531 뮤턴트)
```

스윕은 **별도 worktree** 에서 돈다. 처음엔 소스를 제자리에서 변이시키고 `finally` 에서
되돌렸는데, 스윕이 도는 동안 `git add -A` 를 하는 바람에 **변이 두 개가 커밋에 섞여
들어갔다.** 커밋 둘이 깨진 채로 나갔고, 그 뒤 worktree 를 만드니 31 개가 빨개져서 알았다.

`finally` 는 그 프로세스만 지킨다. **도구가 본체를 건드리는 한 사고는 반드시 난다** —
"조심해서 쓴다" 는 계약이 아니다. 6차 검수가 `openReadOnly` 를 두라고 한 것과 같은 이유다.

살아남은 것을 볼 때는 셋으로 가른다.

- **반례가 없다** → 반례를 쓴다. 이게 대부분이어야 한다.
- **동치 뮤턴트** → 의미가 안 바뀌었다. 그렇다고 적고 넘어간다.
  - 자주 나오는 모양이 하나 있다: **반납이 여러 자리에 있다.** `failAll` · `abortConfig` ·
    루프 머리의 종단 처리 · `releaseHolderSlots` 가 각각 실행권을 놓는다. 그래서 **한
    자리를 지워도 아무것도 안 빨개진다.**
  - 그때는 개별 줄을 쫓지 말고 **계약을 짚어라** — "실패로 닫힌 뒤에는 실행권도 예약도
    남지 않는다". 그리고 **자리를 전부 지웠을 때 빨개지는지**로 확인한다. 그게 "이
    계약이 검사되고 있다" 의 뜻이다.
  - 중복 자체는 나쁜 게 아니다. 6차 ④ 와 16차 ② 가 실행권 고착이었으니 방어가 겹치는
    것은 의도한 것이다. 다만 **뮤테이션 점수는 그만큼 낮게 나온다** — 점수를 코드 품질로
    바로 읽으면 안 되는 이유다.
- **도달 불가** → 지금 경로로는 안 닿는다. **이 결론은 제일 조심해서 내려야 한다** —
  17차에 좌표 CAS 를 그렇게 적었다가 반증당했다. 길이 있는데 못 찾은 것이었다.
  결론 내기 전에 **닿는 시퀀스를 실제로 찾아보고**, 못 찾았으면 "못 찾았다" 고 적어라. `failAll` 의 부분 커밋 보존이 그랬다.
  방어적이라고 적고 넘어가되, "오늘의 사실일 뿐" 이라는 것도 같이 적는다.

### 불변식을 늘릴 때

**끌어올 수 있는지부터 본다.** 외부 효과가 얽힌 반례라도 포기하지 않는다 — 14차의
`pendingActivation` 건이 그랬다. "게시가 무엇인가" 는 상태 술어가 아니지만 "**그 게시의
기준 후보가 어떻게 사라지는가**" 는 순수한 상태 전이였다(I6). 효과 자체와 효과가 상태에
남기는 자국을 구분해라.
...
- **테스트를 코드에 맞춰 고치지 않는다.** 기대가 틀렸으면 왜 틀렸는지 적고 고친다.
  픽스처를 바꿔 통과시키는 것은 고친 것이 아니다.
- **`.toThrow()` 로 끝내지 않는다.** 무엇으로 실패했는지까지 본다 — 엉뚱한 오류도
  통과한다.
- **간헐적으로 깨지는 테스트를 두지 않는다.** 고정 `sleep` 대신 조건 폴링을 쓴다.
  green 이 될 때까지 다시 돌리는 습관이 든다.
...
npm run test:model      # 교차를 생성해서 속성을 때린다
...
| C5.18 | 인증서 자료 없음 | 목록에서 빼지 않는다 (`tests/unit/certs-view.test.ts`) |
...
| C5.24 | GUI HTTP 리스너 put | protocol=http. tls 를 안 붙인다 (`tests/unit/edit.test.ts`) |
| C5.25 | GUI 풀 생성 | pool+backend 두 줄. 빈 풀만은 안 만든다 |
...
| C5.85 | Kit 경로 (로그인 포함) | `gui/src/routes` 에 자리마다 `+page.svelte`. 폴링 없음 |
...
| `tests/unit/surface-freeze.test.ts` (회차 누적) | `--round` 가 **근거를 요구**하고 그것을 `# 회차 N:` 으로 쌓는가 · 번호가 카운터와 맞물리는가 · `--unfreeze` 가 그 줄들도 지우는가 (검수 2026-08-29(3) · B) |
| `tests/unit/install-script.test.ts` | `deploy/install.sh` — systemd 유닛의 재시도 상한이 `[Unit]` 에 있는가(A) · 비밀 파일을 좁은 umask 안에서 쓰는가(B) · `--env` 거절이 안내를 내는가(C). **못 재는 것**: 파일 모드의 전환 창은 경합이라 밖에서 결정적으로 못 만든다 — 여기서 재는 것은 "넓게 만들 수 없는 형태" 이지 "창이 없다" 가 아니다 |
| `tests/store/secrets-pg.test.ts` | `PgSecretStore` — 왕복·내용 주소 멱등·**평문 부재**·KEK 불일치·**AAD 참조 결박**·오라클 방지·`FsSecretStore` 와 같은 참조·사실 캐시·GC 정책 공유 (§4.8.1). 검수 2026-08-29 로 셋이 더 붙었다: **재업로드가 못 여는 행을 고친다**(A) · **digest 를 자료에서 다시 잰다**(B) · **캐시가 밖에서 안 썩는다**(C) |
