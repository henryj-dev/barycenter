...
`listener create --protocol http|tcp` 은 changeset 을 지나 **commit 까지** 한다.
apply 는 아니다. tcp 는 `defaultPool` 이다 — `http.defaultAction` 이 아니다.
udp·https 와 풀·라우트 명령은 아직 패치를 안 만든다. 드레인은 S2 다.
...
| **v0.4** CLI | export/import ✅ · 나뉜 changeset 단계 ✅ (`changeset new\|patch\|plan`, `commit --plan`, `apply --plan`). HTTP·TCP listener create ✅. udp·https·풀·라우트 명령은 아직 | 같은 매니페스트를 두 번 import 해도 결과가 같다. `apply --plan` 은 changeset 을 안 연다 |
