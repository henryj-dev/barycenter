...
**아직 미정으로 남기는 것.** `CertificateOrder`·`Challenge` 의 durable 스키마, 갱신
스케줄러의 시점 정책(만료 30 일 전 + 지터), 레이트리밋 백오프, EAB, dns-01 프로바이더
인터페이스. 이것들은 주문 상태기계를 실제로 짓는 회차에 정한다 — 지금 정하면 안 짜 본
것을 규범으로 굳히게 된다.
