...
> 이건 "나중에" 가 아니라 **결정**이다. 잴 수 없는 것을 넣으면 켰다는 사실만 남는다.
>
> S9(SNI 3분기)를 여기에 같은 종류로 적어 두었었는데, **그건 틀렸다** — S9 는 잴 수
> 있었고, 재 보니 갈렸다(2026-08-23). 두 항목의 차이는 이렇다: 스테이플은 **재는 도구가
> 있는데 잴 대상이 없고**(CA 가 OCSP 를 안 준다), S9 는 **잴 대상이 있는데 안 재고
> 있었다.** 앞의 것만 "현행 유지" 의 근거가 된다.
...
| `on_no_sni` | `reject` \| `pool(pool_id)` — **TLS 는 맞는데 SNI 가 없다.** S9 로 열렸다 |
| `preread_timeout` | ClientHello 대기 |

**파싱할 수 없으면(비-TLS·malformed) 무조건 `reject` 다. 설정할 수 없다.**
...
| **SNI 없음 (TLS 이지만)** | `on_no_sni` (설정 가능) | S9 가 갈린다는 것을 실측했다 (2026-08-23) |
| **비-TLS / 파싱 실패** | **reject 고정** | TLS 패스스루 포트에 온 비-TLS 바이트를 어디로도 보내지 않는다 |

> `spike/s9` 가 손으로 만든 ClientHello 를 흘려 셋이 서로 다른 결과를 내는 것을 봤다:
>
> | 입력 | 결과 |
> |---|---|
> | TLS, SNI 없음 | `$ssl_preread_protocol` 이 **차 있다** → 별도 분기 |
> | malformed ClientHello | preread 가 DECLINED → protocol 이 비어 **비-TLS 와 한 통** |
> | preread timeout | nginx 가 **연결을 끊는다** → 분기에 안 온다 |
>
> **malformed 가 비-TLS 와 같은 통으로 가는 것이 이 승격을 안전하게 만든다.** 위 표의
> 마지막 행이 지키려던 것 — "TLS 로 안 읽히는 바이트를 어디로도 안 보낸다" — 이 컬럼
> 하나가 늘어난 뒤에도 그대로 산다. 갈리지 않았다면 승격하면 안 됐다.
>
> **`on_no_sni` 를 안 걸면 map 도 하나만 렌더한다** — 쓰지 않는 분기를 미리 만들지 않는다.
> `'reject'` 를 명시해도 마찬가지다: 동작이 기본과 같다.
...
#### 4.8.1 두 번째 드라이버 — `PgSecretStore` (2026-08-28)

`FsSecretStore` 는 DP 호스트 파일시스템에 **평문으로** 쓴다. 파일 헤더가 스스로
*"암호화가 아니다"* 라고 적었고, 로드맵 R-1 이 그 자리를 *"KMS/Vault 드라이버"* 로
비워 뒀다. 남는 위험은 **호스트 침해 시 즉시 노출**과 **백업·스냅샷·디스크 복제본
잔존**이다. 여기서 그 자리를 채우되, 벤더 API 가 아니라 **이미 있는 PG** 로 채운다.

**평문 금지 조항은 유지된다.** DB 에 들어가는 것은 봉투 암호화의 **암호문**이다:

| DB 가 드는 것 | DB 가 안 드는 것 |
|---|---|
| AES-256-GCM 암호문 · nonce · auth tag | 평문 개인키 |
| 자료를 감싼 **DEK** 를 KEK 로 감싼 것 | **KEK** — env(`BARY_SECRET_KEK`) 또는 KMS 다 |
| `sha256` · `chain_digest` · `key_digest` · `facts` | — |

**DB 덤프 하나로는 키가 안 나온다.** 그것이 "평문으로 두지 않는다" 의 실질이고,
`FsSecretStore` 의 `0400` 보다 강하다 — 파일 권한은 호스트를 잡은 상대에게 아무것도
아니지만, KEK 가 DB 밖에 있으면 덤프를 가져간 상대는 암호문만 든다. 자료마다 DEK 를
따로 쓰므로 **KEK 회전은 DEK 재감싸기**이고 자료 바이트를 다시 읽지 않는다 —
그래서 `kek_id` 열을 처음부터 든다. 회전 절차 자체는 이 회차의 범위가 아니다.

**KEK 가 없으면 이 드라이버는 안 뜬다.** 없는 것을 기본값으로 지어내면 "암호화된 줄
알았다" 가 그대로 돌아온다 — 기동에서 죽는 편이 정직하다.

**드라이버 선택은 배포의 것이다.** `BARY_SECRET_BACKEND=fs|pg`. 기본은 `fs` 다 —
전용 VM 한 대(§11.3 의 v1 권장 배포)에서는 KEK 를 어디 둘지가 새 문제이고,
그 결정을 안 한 배포를 조용히 바꾸지 않는다.

##### 동기 경계 — `facts()` 만 동기다

`SecretStore` 의 일곱 메서드는 전부 동기였다. PG 는 아니다. 전부를 async 로 넓히면
**동기 경로 둘**이 물든다:

| 자리 | 무엇 |
|---|---|
| `ConfigStore` 의 SAN 커버 검증 | 커밋 앞에서 도는 **동기 검증기** |
| plan 의 `certificateChanges` | 임팩트를 만드는 **순수 함수** |

그래서 가른다. **`facts()` 는 동기로 남는다** — 그것이 다루는 값(`notAfter`·SAN·
발급자)은 **비밀이 아니고**, 이미 `put` 시점에 뽑아 옆에 적어 두는 캐시다. 나머지
여섯(`put`·`get`·`describe`·`putKey`·`getKey`·`listRefs`)은 **자료 바이트를 만지므로**
async 다. 여섯의 호출부는 전부 이미 async 컨텍스트 안이라 이 선이 실제로 안 아프다.

**캐시 miss 는 「사실을 모른다」다.** 기존 의미와 같다 — 검증기는 넘어가고 만료 게이지는
그 인증서를 안 싣는다. 없는 사실을 0 으로 채우지 않는다(§4.6 과 같은 규칙). 캐시는
기동에서 전부 적재하고 `put` 이 갱신하며 주기적으로 다시 읽는다.

#### 4.8.2 KEK 를 어디서 받나 — 이음매 하나, 벤더 없음 (2026-09-01)

§4.8.1 의 표가 KEK 를 *"env(`BARY_SECRET_KEK`) 또는 **KMS**"* 라고 적어 두고 뒤쪽을
비워 뒀다. 로드맵 R-1 의 잔여가 그것이다 — *"KEK 를 KMS 에서 받는 것이 다음 단계다."*

**세 번째 드라이버가 아니다.** `PgSecretStore` 는 이미 `kek: Buffer` 를 받는다. 바뀌어야
하는 것은 자료를 어디 두느냐가 아니라 **그 버퍼가 어디서 오느냐**뿐이다. 저장소 드라이버를
하나 더 만들면 `put`·`get`·GC·사실 캐시를 통째로 다시 구현하게 되는데, 그중 어느 것도
KMS 때문에 달라지지 않는다.

##### 벤더 SDK 를 안 들인다

이 저장소는 **의존성 0 으로 성립한다**(§11.2). ACME 의 JWS 도 CSR 도 직접 만들었고,
엔진 통합은 SDK 가 아니라 이음매다 — `BARY_RELOAD_CMD` · `BARY_CONFIGTEST_CMD`,
그리고 DNS-01 의 파일 훅 계약. `PgSecretStore` 자신이 *"벤더 API 가 아니라 이미 있는
PG 로 채운다"* 로 선 자리다.

그래서 여기서도 이음매를 판다:

    BARY_SECRET_KEK        값을 그대로 준다 (지금까지의 길)
    BARY_SECRET_KEK_CMD    명령을 돌려 그 stdout 을 KEK 로 쓴다 (새로 파는 길)

한 줄이면 무엇이든 붙는다 — `aws kms decrypt …`, `vault read -field=… `,
`gcloud kms decrypt …`, `systemd-creds cat …`, TPM 을 읽는 스크립트. **벤더를 고르지
않는다**는 것이 요점이다: SDK 를 하나 들이면 그 벤더를 안 쓰는 배포도 그 코드와 그
공급망을 진다.

얻는 것이 실질적이다 — `BARY_SECRET_KEK` 이면 키가 `$PREFIX/env`(0640)에 **파일로
남지만**, `_CMD` 면 기동 때 한 번 가져와 **메모리에만** 있다. 덤프·백업·디스크 복제본에
키가 안 남는다는 §4.8.1 의 주장이 env 파일까지 확장된다.

##### 규칙 넷

**① 둘을 같이 주면 안 뜬다.** 한 비밀에 출처가 둘이면 어느 쪽이 이기는지가 곧 사고다 —
`install.sh` 의 `MANAGED_ENV_KEYS` 가 같은 이유로 같은 키를 두 번 못 넣게 한다. 골라서
이기게 하는 것보다 **거절하는 것**이 낫다.

**② 실패하면 안 뜬다.** §4.8.1 의 *"KEK 가 없으면 이 드라이버는 안 뜬다"* 와 같은 규칙이다.
명령이 죽거나, 빈 것을 내거나, 32 바이트가 아니면 기동에서 죽는다. 지어내면
*"암호화된 줄 알았다"* 가 그대로 돌아온다.

**③ 기동에 한 번만 부른다.** 자료마다 부르면 KMS 호출이 자료 수만큼 늘고, 실패 표면이
「기동」에서 「모든 인증서 읽기」로 넓어진다. KEK 회전은 `kek_id` 가 이미 받치고 있고
**회전 절차 자체는 여전히 범위 밖이다**(§4.8.1 과 같은 판정).

**④ stdout 은 비밀이다.** 로그·오류 메세지에 안 싣는다. **명령 문자열 자체도 안 싣는다** —
거기 키를 박아 둔 배포가 있을 수 있고, `execFile` 이 만드는 오류 메세지는 명령을 통째로
담는다(2026-09-01 실측). 실패하면 **어떻게 죽었는지**(종료 코드·시그널·상한 초과)와
**stderr** 만 낸다.

⚠️ **그래서 명령은 키를 stdout 으로만 내야 한다.** stderr 는 진단 채널이고 **로그로
나간다** — 거기 키를 쓰는 명령은 그것을 로그에 쓰는 것이다. stderr 를 빼면 `AccessDenied`
와 `network unreachable` 이 똑같이 「종료 코드 1」이 되고, 그건 데몬이 안 뜨는 날 제일
필요한 정보를 없애는 것이라 **싣기로 한다.** 대신 그 계약을 여기 적는다.

시간 상한을 건다. KMS 가 안 답하는 날 기동이 영원히 매달리면, 그건 「안 뜬다」가 아니라
**「떴는지 모른다」**이고 그 둘을 섞지 않는 것이 이 저장소의 규칙이다(§6.2).

#### 4.8.3 KEK 회전 — DEK 를 다시 감싼다 (2026-09-01)

`023_secret_materials.sql` 이 `kek_id` 열을 처음부터 들면서 적어 뒀다:
*"회전 절차 자체는 아직 없다."* 그 빈자리의 대가는 **유출과 분실이 같은 값**이었다 —
둘 다 남는 길이 「인증서를 전부 다시 올린다」 하나뿐이었다.

그리고 그 길은 전부에게 열려 있지도 않다. `PgSecretStore.put` 의 주석이 적어 둔 그대로다:
...
봉투가 두 층이라 회전이 닿는 곳은 하나다:

    wrapped_dek = kekNonce(12) || kekTag(16) || KEK 로 감싼 DEK   ← 이것만 바꾼다
    ciphertext  = tag(16) || DEK 로 감싼 자료                      ← 안 건드린다

그래서 **자료 바이트를 복호화하지 않는다.** 자료마다 DEK 를 따로 쓴 것이 여기서
값을 낸다 — 자료가 아무리 커도 회전은 DEK 한 겹이다. AAD 는 두 층 모두
`<scheme>://<name>@<version>` 이라 행에서 그대로 세운다.

##### `kek_id` 가 진행 상황이다

한 행씩 트랜잭션으로 처리하고, 다시 돌리면 **아직 옛 id 인 행만** 집는다. 중간에
죽어도 이어받는다 — 그것이 이 열을 처음부터 든 이유다.

그래서 **새 id 는 옛 id 와 달라야 한다.** 같으면 「어디까지 했나」를 물을 방법이
사라진다. 같은 KEK 로의 회전도 막는다 — 아무 일도 안 하면서 열만 바꾸면 그 대답이
거짓이 된다.

##### 못 여는 행은 실패로 낸다

옛 KEK 로도 안 열리는 행이 있을 수 있다(다른 KEK 환경의 덤프를 복원했거나 자료가
변조됐다). **건너뛰고 초록을 내지 않는다** — 그러면 운영자는 회전이 끝났다고 믿고
재기동한다. 나머지는 그래도 옮기고, 못 옮긴 것을 참조로 세어 빨갛게 끝낸다.
한 행 때문에 전부를 멈추지도 않는다.

##### ⚠️ 데몬은 회전을 모른다

돌고 있는 데몬은 기동 때 받은 **옛 KEK 를 메모리에 들고 있다.** 순서가 있다:

    ① 회전을 돌린다 (`--check` 로 먼저 본다)
    ② KEK 출처를 새 것으로 바꾼다 (env 또는 KMS)
    ③ 데몬을 재기동한다

①~③ 사이에 **제어 평면이 인증서를 못 읽는다.** 데이터 평면은 멀쩡하다 — nginx 는 이미
구워진 세대를 서빙하고 그 안에 자료가 파일로 있다. 창을 없애려면 데몬이 두 KEK 를 동시에
들어야 하고, 그건 이 회차의 범위가 아니다. **창이 있다는 것을 적어 두는 편이**
없는 척하는 것보다 낫다.

##### 자리가 스크립트인 이유

`scripts/rotate-kek.mjs` 다. API 오퍼레이션이 아니다 — **새 KEK 를 API 로 보내는 것**이
바로 §4.8 이 피하는 모양이다. 운영자가 DB 와 두 KEK 에 닿는 자리에서 한 번 도는 일이고,
`scripts/token.mjs` 와 같은 부류다.

KEK 를 읽는 규칙은 `resolveKek`(§4.8.2) 한 자리를 그대로 쓴다 — 새 KEK 도 같은 함수에
이름만 바꿔 태운다. 「값과 명령을 둘 다 주면 거절」·「32 바이트가 아니면 거절」이
공짜로 따라온다.
...
인증 실패 제한은 제어 API 인스턴스별 메모리에서 한다. 키는 프록시 헤더를 신뢰하지 않고
TCP peer의 `remoteAddress`만 쓴다. 10분 동안 보관하며 키 수는 1024개로 제한한다. 같은
키에서 5회 실패하면 `429`와 `Retry-After`를 내고, 지연은 1초부터 최대 60초까지 지수적으로
늘어난다. 루프백도 제외하지 않는다 — 로컬 CLI가 잘못된 토큰을 반복해도 제어 평면을
압박하지 않아야 하며, 올바른 인증이 성공하면 해당 키의 실패 상태를 지운다. 여러 인스턴스의
전역 합산은 하지 않는다. 제어 평면 대수와 DB 쓰기 비용을 고려한 의도적 범위다.
...
bary listener create --name game --protocol tcp --bind 0.0.0.0 --port 999 --pool pool-a
bary listener create --name dns --protocol udp --bind 0.0.0.0 --port 53 --pool pool-a --preset dns
...
`listener create` 는 changeset 을 지나 **commit 까지** 한다. apply 는 아니다.
tcp·udp 는 `defaultPool` 이다. udp 는 named preset 만 받는다.
https 는 `tls.policy` 와 `tls.defaultCertificate` 가 필수다. http2 는 안 적는다.
패스스루는 `tls_passthrough` 다. tls 를 안 붙인다. unmatched SNI 풀은 선택이다.
`pool create` 는 첫 백엔드와 같이 넣는다. 빈 풀만은 plan 이 막는다.
알고리즘은 `round_robin` 과 `hash` 다. hashKey 는 검증기와 같은 화이트리스트다.
tcp·udp 는 `remote_addr` 만. `source_ip_hash` 는 hashKey 를 안 붙인다.
`route create` 는 HTTP 호스트 → 풀 proxy, URL redirect, reject, 또는 패스스루다.
websocket 은 HTTP proxy 에서만 켠다. 안 적으면 끈다.
...
> 위는 v0 이 실제로 내는 모양이 **아니다** — `default` 하나로 다 받으면 파싱 실패까지
> 폴백 풀로 간다. 우리 렌더는 빈 문자열을 따로 분기한다(§4.1).
>
> S9 가 그 별도 분기를 실측했다(2026-08-23): 빈 SNI·malformed·preread timeout 이 **안정적으로
> 구분된다.** `$ssl_preread_protocol` 이 no-SNI 에서 차 있고 malformed 에서 비며, timeout 은
> 아예 분기에 안 온다(연결이 끊긴다). 그래서 `on_no_sni` 는 축소되지 않고 **설정 가능으로
> 승격**됐다. 파싱 실패는 계속 고정이다.
...
#### 7.3.1 가중치 — **슬롯 목록의 반복으로 낸다** (검수 D2, 2026-08-24)

`Backend.weight` 는 정적 `server` 줄 경로에서는 `weight=N` 으로 나가는데, **멤버십
평면에서는 아무도 안 읽었다.** `slotsOf` 가 `host:port` 만 실었고 `pickExpression` 의
셋(round_robin · hash · least_conn)이 전부 균등하게 골랐다. 즉 **같은 모델이 엔진에
따라 다르게 밸런싱됐고**, 사용자는 저장에 성공하고 GUI·CLI 는 값을 되돌려 주고 plan 도
아무 말을 안 했다. 이 저장소가 `source_ip_hash` 에서 이미 한 번 잡은 부류다.

**세 안 중 ㉡ 을 고른다.**

| 안 | 대가 |
|---|---|
| ㉠ 슬롯에 가중치를 싣는다 (`host:port\|w`) | admin 와이어 문법이 넓어지고, **`in:`·드레인 관측이 쓰는 `host:port` 키와 갈린다** — 접미사를 벗기는 자리가 네 곳으로 는다 |
| **㉡ 목록을 가중치만큼 반복** | **와이어가 안 바뀐다.** dict 값이 길어진다 |
| ㉢ 안 고치고 plan 이 경고 | 거짓말은 아니게 되지만 **가중치는 여전히 안 걸린다** |

㉠ 을 안 고른 이유가 결정적이다: peer 문자열은 슬롯에만 사는 것이 아니라
`in:<peer>` 카운터와 `/membership/inflight` 의 질의 키이기도 하다. 거기에 접미사가
생기면 **벗기는 자리가 넷**이 되고, 이 저장소가 반복해서 물린 *"자리가 둘이면 언젠가
갈린다"* 를 새로 만드는 셈이다.

**㉡ 의 대가(dict 크기)는 이 회차에 관리 가능해졌다.** D4 가 퇴역 epoch 회수를 붙였고
`bary_membership_slot_keys` 게이지가 자라는 것을 보여 준다. 그래도 상한은 필요하다 —
해독기가 `weight` 를 **1..1,000,000** 으로 받기 때문이다(그 범위를 좁히지 않는다:
`modelAt` 이 옛 리비전을 같은 해독기로 읽으므로 좁히면 **롤백이 막힌다**, 검수 D7).

그래서 **확장에 상한을 건다**:

1. 풀 안의 가중치를 **최대공약수로 나눈다** — `2:4` 는 `1:2` 다. 낭비가 없다
2. 합이 상한(`SLOT_EXPANSION_CAP`)을 넘으면 비율을 유지한 채 줄인다.
   **모든 peer 가 최소 1 칸을 갖는다** — 줄이다가 백엔드를 조용히 빼면 그건
   밸런싱이 아니라 장애다
3. 사본을 **고르게 섞는다.** 뭉쳐 두면 `round_robin` 이 무거운 peer 에게 연속으로
   몰아준다 — 비율은 맞고 버스트가 생긴다

**가중치가 전부 1 이면 산출물이 지금과 글자 그대로 같다** — GCD 가 1 이고 사본이
하나씩이라 정렬된 목록이 그대로 나온다. 안 쓰는 배포의 거동이 안 바뀐다는 뜻이다.

`least_conn` 은 이 방법과 자연스럽게 맞물린다: 사본들이 같은 `in:<peer>` 키를 보므로
동점 집합에 무거운 peer 의 사본이 더 많이 들어가고, 라운드로빈 tie-break 이 그만큼 더
고른다. `hash` 는 해시 공간에서 차지하는 칸이 늘어난다 — 그것이 가중 해시의 뜻이다.
...
4. **`strict_priority` 모드는 옵트인**이다 — **구현됨 (2026-08-23)**,
   `HttpProfile.strictPriority`. 충돌 그래프의 연결 요소 전체(정확일치 포함)를 anchored
   정규식으로 내려 등장 순서로 정렬한다.

   **연결 요소 통째로 내리는 것이 핵심이다.** 한 쌍만 내리면 그 둘과 겨루는 셋째가
   앞 클래스로 남아 무조건 먼저 평가된다 — 조용히 틀린다.

   **역전이 없는 요소는 안 내린다.** 내려 봐야 순서가 같고 정규식은 공짜가 아니다.

   ⚠️ **S10 은 자기 기준을 못 넘었다.** §12.0 이 "라우트 500개 p99 영향 < 5%" 를
   요구했는데 `spike/s10` 실측은 이렇다:
   ...
   선형이다 — 정규식은 순차 평가라 강등 하나마다 비용이 붙는다. §12.0 의 실패 규칙은
   "모드 미제공" 이지만, 이 절이 같은 모드를 두고 **"라우트 수 상한과 벤치 기준을 함께
   정의한다"** 고 적어 두었다. 그래서 **모드는 내되 상한을 강제한다**:
   `STRICT_PRIORITY_MAX_LOWERED = 128` (5% 예산 ÷ 강등당 ~0.037%p ≈ 135 에서 여유를
   두어 자른다). 넘으면 검증기가 `strict_priority_too_many` 로 막는다.

   상한 없이 내면 500 라우트짜리 배포가 조용히 두 배의 대가를 물고, 그게 §12.0 이
   막으려던 바로 그것이다.

   **스파이크는 게이트에 안 넣는다** — 기준을 못 넘는 것을 알면서 넣으면 빨강을 무시하는
   법을 익히게 된다. S20 과 같은 처리다. `./spike/s10/run.sh` 로 직접 돌린다.
...
#### 8.2.1 계정을 만드는 자리 — **REST** (검수 D21, 2026-08-24)

`acme_accounts` 에 넣는 코드는 `AcmeStore.upsertAccount` 하나였고 **호출자가
테스트뿐이었다.** 계정이 없으면 `AcmeRunner.scan` 이 `acme.no_account` 를 경고로 찍고
건너뛰므로, 새 배포에서 운영자가 인증서에 `acme` 의도를 적어도 **주문이 영영 안 열린다.**
로그 한 줄 말고는 아무 신호가 없다 — **ACME 기능 전체가 제품 표면에서 도달 불가였다.**

도달성 게이트를 클래스 메서드까지 넓힌 회차(G6)에 켜자마자 짚었다.

**세 안 중 REST 를 고른다.**

| 안 | 대가 |
|---|---|
| **`POST /api/v1/acme/accounts`** | **B 동결 드리프트다** — 라우트 표가 움직인다 |
| `bary acme account create` | 표면이 CLI 에만 산다. **GUI 에서는 못 만든다** |
| 기동 환경변수 | 표면이 안 움직인다. 계정이 **설정이 아니라 배포**가 된다 |

**CLI 만 두는 것은 제품 명제와 어긋난다.** §2 가 이 제품을 GUI 로 세웠는데, 인증서
자동 갱신을 켜려면 셸에 들어가야 한다면 그 명제가 그 자리에서 깨진다. 그리고 REST 를
내면 CLI·GUI 가 **같은 길**을 쓴다 — 자리가 하나다.

**환경변수는 계정을 리비전·감사·롤백 밖으로 내보낸다.** 계정은 개인키를 들고 CA 와의
신원을 정한다. 그것이 배포 아티팩트가 되면 「누가 언제 만들었나」가 어디에도 안 남고,
바꾸려면 재기동해야 한다. §4.8 이 자료를 리비전에 결박한 이유가 그것이다.

**B 표면이 움직이는 것은 대가이지 반대 근거가 아니다.** 그 게이트는 표면이 느는 것을
**보이게** 하려고 있지 막으려고 있는 것이 아니고, `freeze-b.mjs --write` 가 그 길이다.
없는 기능을 도달 가능하게 만드는 값이 라우트 둘의 값보다 크다.

**등록은 여기서 안 한다.** 이 창구는 원장에 계정을 적고 키를 만들 뿐이고, CA 등록
(`newAccount`)은 러너가 **첫 주문 때** 한다(`#drive` 가 `accountUrl` 이 없으면 부른다).
창구가 CA 를 기다리면 CA 가 느린 날 계정을 못 만들고, 그건 **관측 못 한 것을 실패로
접는 것**이다.

**목록 창구도 함께 낸다.** 만들 수만 있고 확인할 수 없으면 「제품 경로가 있다」가 절반만
참이다 — 운영자도 GUI 도 「계정이 있는가」를 물을 자리가 필요하다. 개인키 참조는 안 낸다.
...
| 백엔드 디스커버리 (S14 대안 B) | **S14 부분 통과** — 회차마다 6/8~8/8. **8/8 이 나오는 회차가 있다**(SRV·기존 세션 포함). 흔들리는 것은 엔진이 아니라 **스파이크의 측정 결합**이다 (2026-08-23 재확인, 아래). 실패 모드 표는 `DataplaneCapabilities.nativeDns` 로 표면화. 모델에 선택형 없음 | v0.7 |
...
- 열린 화면: Plan·Impact · Listeners · Pools · Routes · Certificates · Status · Rendered · Audit · Login.
  ...
  Audit 는 `GET /api/v1/audit` 다. 로그다. before/after 본문은 API 가 안 준다.
  폴링하지 않는다.
  ...
  `source_ip_hash` 다. hashKey 는 검증기와 같은 화이트리스트다. source_ip_hash 는
  hashKey 를 안 붙인다. HTTP 라우트 `put`/`delete` 는 호스트 → 풀 proxy 다.
...
- CP ↔ DP Agent 의 **원격 전송이 생겼다** (2026-08-23) — gRPC 가 아니라 **mTLS HTTP/JSON**
  이다. `RemoteDataplaneDriver`(CP 쪽) ↔ `createDpAgentServer`(DP 쪽), 진입점은
  `bary-dp-agent`. `BARY_DP_REMOTE_URL` 을 주면 `barycenterd` 가 그쪽으로 간다.

  gRPC 를 안 고른 이유는 여덟 메서드에 스트리밍이 하나도 없기 때문이다 — 요청/응답뿐인
  계약에 코드 생성과 런타임 의존을 더하면 얻는 것보다 지는 것이 많다.

  **가르는 것은 CP↔에이전트뿐이다.** 위 실측(*"에이전트와 nginx 는 같은 파일시스템을 봐야
  한다"*)은 그대로다 — 에이전트는 여전히 nginx 옆에서 돌고, 원격으로 가면 그쪽 호스트에서
  `bary-dp-agent` 로 선다.

  ⚠️ **두 실패를 절대 안 섞는다.** 이 저장소가 반복해서 배운 *"관측하지 못한 것과 관측해서
  아니라고 한 것은 다르다"* 가 여기서 두 방향으로 산다:

  | | 뜻 | 전송 |
  |---|---|---|
  | `DpRejection` | 에이전트가 **판정했다** — 낡았다·이미 끝났다 | **409 만**. 본문에 `kind` |
  | `RemoteDpUnreachable` | **못 물었다** — 세계에 대해 주장하지 않는다 | 그 외 모든 비-200 |

  섞으면 5xx 가 곧 전환 포기가 되거나, 낡은 리더가 거절을 일시적 장애로 읽고 재시도한다.

  인증은 mTLS 에 **CN 화이트리스트**를 얹는다 — CA 를 나눠 쓰는 배포에서 "이 CA 의 아무
  인증서" 는 너무 넓다. `rejectUnauthorized` 를 끄는 길은 안 만든다.
...
- **제어 API의 외부 평문 바인드는 기본 거부**다. 기본 `BARY_LISTEN`은 루프백이지만,
  컨테이너 내부에서는 포트 퍼블리시를 위해 `0.0.0.0`이 필요할 수 있다. 앞단 프록시가 TLS를
  종단하는 배포는 데몬이 그 사실을 관측할 수 없으므로 `BARY_ALLOW_PLAINTEXT_EXPOSED=1`을
  명시해야 한다. 이 플래그는 안전을 보장하는 설정이 아니라 평문 경계를 인지했다는 선언이며,
  허용해도 기동 경고는 남긴다. 허용 상태는 인증된 `/metrics`의
  `bary_api_plaintext_exposed` 게이지로 관측한다.
...
**그 결정은 그대로다. 다만 어느 배포를 고르든 운영자에게 같은 것 하나가 필요하다** —
*"이 설정이 어느 (전송, 주소, 포트) 를 잡는가."* 그래서 `GET /api/v1/sockets` 와
`bary get sockets` 를 낸다 (2026-08-23). 없으면 `/listeners` 를 받아 프로토콜→전송
변환을 밖에서 다시 구현해야 하고, 그러면 §4.5 의 규칙이 두 벌이 된다.
  ...
  · 리스너 하나가 소켓 **여럿**일 수 있다 (S20 — quic 은 UDP 도 잡는다). h3 를 여는 날
    이 뷰의 행이 저절로 는다.
...
| S9 | SNI 결과 3분기 관측성 | ✅ **통과 (2026-08-23).** 셋이 서로 다르다 — no-SNI 는 `$ssl_preread_protocol` 이 차고, malformed 는 비-TLS 와 한 통이고, timeout 은 연결이 끊긴다. `on_no_sni` 를 설정 가능으로 승격했다 | (해당 없음) |
| S10 ❌ | 라우트 컴파일러 | **기준을 못 넘었다 (2026-08-23).** 강등 50개 +3.4% · 250개 +9.8% — 선형이고 500 라우트에서는 기준의 두 배다. §7.5-4 의 *"라우트 수 상한"* 으로 받았다: 모드는 내되 **강등 128개 상한**을 검증기가 강제한다 | `strict_priority` 모드 미제공 |
...
| S14 ~ | **대안 B 실증** | 여덟 점 전부가 **어느 회차에는 초록이다**(SRV·기존 세션 포함, 2026-08-23). 다만 회차마다 6/8~8/8 로 흔들린다 — 원인은 엔진이 아니라 스파이크가 한 resolver·한 존을 여덟 측정에 나눠 쓰는 것이다. 게이트에 안 넣는다 | 폴백 자체가 없음 → 요구 재조정 |
...
> 기록으로 남는다.
>
> ✅ **이제 측정이다** (2026-08-23). 위 문단은 오래 이렇게 끝났다: *"재투영을 빼는 변이가
> 아무 테스트도 안 깨뜨린다 — 그걸 밖에서 결정적으로 만들 방법이 없다."*
>
> **"밖에서" 가 그 문장의 전부였다.** 헬스는 프로버의 소유가 아니라 `backend_health`
> 표의 소유이고 `eligible()` 은 그 표를 읽는다. 가짜 드라이버의 `applyConfig` 안에서
> 표를 쓰면 그 순간이 곧 staging 과 활성화 **사이**다.
>
> `tests/store/reproject-window.test.ts` — 죽는 경우 · 살아나는 경우 · 드레인 · 멱등 ·
> 재투영 실패 다섯. **재투영을 빼는 변이에 다섯이 전부 죽는다.**
>
...
7. **`least_conn` 을 v1 계약에 넣을 것인가** — ✅ **넣었다** (2026-08-23). 배제 근거였던
   "워커별 근사" 가 사실이 아니게 됐다 — `in:` 카운터가 `lua_shared_dict` 에 살아 워커
   간 공유다. 동점은 라운드로빈으로 가른다(nginx 네이티브와 같다). 골든이 §12.0 의
   합격 기준(균등 부하 편차 < 10%)을 실물로 넘겼다.
8. **SNI 결과 분기 수** — ✅ **3분할로 갔다** (2026-08-23). S9 가 실측했다.
   `on_no_sni` 가 설정 가능해졌고, **파싱 실패는 계속 고정**이다 — 그 통이 no-SNI 와
   갈리기 때문에 하나만 열 수 있었다.
9. **`strict_priority` 제공 여부** — ✅ **옵트인으로 제공한다** (2026-08-23). 다만
   S10 이 기준을 못 넘어서 **강등 128개 상한**이 함께 붙는다 (§7.5-4).
...
| 항목 | v1 | v2 |
|---|---|---|
| 전역 숫자 priority | nginx 우선순위 위에 완전 재현 | **매치 클래스 안에서만이 기본.** `strict_priority` 옵트인으로 재현하되 **강등 128개 상한** — S10 이 500 라우트 기준을 못 넘었다 |
| `least_conn` | 기본 알고리즘 + "근사 명시" | **v0 기본에서 제외.** S6 결과에 따름 |
| 드레인 | GOAWAY + `force_close` 약속 | **관측 + 새 트래픽 차단까지.** 강제 종료는 capability |
| SNI 결과 분기 | 3분할 확정 | **3분할.** S9 가 통과했다 (2026-08-23) — 다만 v2 와 달리 파싱 실패는 계속 고정이다 |
...
| SNI 분기 | 2분할이되 `on_no_sni` 를 설정 가능하게 둠 | **둘 다 설정 가능** (S9 뒤). 파싱실패만 `reject` 고정. `on_no_sni` 를 안 걸면 렌더는 여전히 map 하나 |
