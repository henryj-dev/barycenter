...
> - **S8 통과 (2026-08-11).** 세대 결박이 롤백에서 TLS 까지 되돌린다. 같은 하네스로 v0/v1
>   설계가 왜 되돌리지 못했는지도 재현했다.
> - **v0.1 타입·API·DB 스키마 freeze: 여전히 No-Go.** 남은 block 은 **S12**(크래시 저널)
>   하나이고, S5·S11 의 프로토콜 잔여분(평면 부분 전환 · cut→replay)과 S13(GC) 이 남아 있다.
