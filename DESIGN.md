...
`listener create --protocol http|tcp|udp` 은 changeset 을 지나 **commit 까지** 한다.
apply 는 아니다. tcp·udp 는 `defaultPool` 이다. udp 는 named preset 만 받는다
(`dns`·`wireguard`·`game_generic`·`custom`). 모르는 preset 은 패치를 안 만든다.
https 와 풀·라우트 명령은 아직이다. 드레인은 S2 다.
...
| **v0.4** CLI | export/import ✅ · 나뉜 changeset 단계 ✅ (`changeset new\|patch\|plan`, `commit --plan`, `apply --plan`). HTTP·TCP·UDP listener create ✅. https·풀·라우트 명령은 아직 | 같은 매니페스트를 두 번 import 해도 결과가 같다. `apply --plan` 은 changeset 을 안 연다 |
