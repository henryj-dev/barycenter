...
| TLS 종단 · `SecretStore` · ACME · 인증서 세대 롤백 | S8·S16·S17·S18 | **v0.6 엔진은 열렸다.** 업로드 종단·SNI·policy·갱신 틱·롤백. GUI 정책·https 포트는 있다. 자료 업로드 폼·주문 GET 은 없다. OCSP 는 **안 짓는다** (§4.6.1). GC 원장은 **안 짓는다** (S13) |
...
- 열린 화면: Plan·Impact · Listeners · Pools · Routes · Certificates · Status.
  ...
  호스트 라우트는 SNI 바인딩이 따로 필요하다. commit 까지 하고 apply 는
  Impact 가 한다.
...
| **v0.6** TLS | 업로드 종단 · SNI · 정책 · 롤백 · ACME http-01 러너 ✅. GUI 정책·https 포트 ✅. 주문 GET · dns-01 · 자료 업로드 폼 · SNI 바인딩 폼은 아직 | 무중단 갱신 틱 + 롤백 시 옛 자료 |
...
| GUI | v0.5 에 8화면 | **3화면으로 시작.** 지금 여섯 화면 + HTTP/TCP/UDP/HTTPS 쓰기. Kit·드레인·자료 업로드 폼은 아직 |
