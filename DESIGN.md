...
- **동시 멱등성이 없다.** 같은 오퍼레이션 6개를 동시에 보내면 전부 성공하고 **HUP 이 6회** 나간다. 예약은 슬롯만 독점할 뿐 apply 실행권을 독점하지 않는다.
- **`partially_activated` 가 종단 상태다.** §6.2 #8 은 "재시도" 라고 하는데 `recover()` 는 같은 상태를 돌려주고 끝낸다. 못 넘어간 평면의 예약이 남는다.
- **FileStore 락을 우회할 수 있다.** `openUnlocked().save()` 가 주인이 있는데도 덮어쓴다. 놓은 handle 도 계속 쓴다. 새 주인이 죽은 락을 회수한 뒤 옛 handle 의 `release()` 가 그 락을 지워 제3 writer 가 열린다.
- **좌표 문자열이 정규화되지 않는다.** `'1'` 과 `'01'` 이 서로 다른 슬롯을 잡는다.
