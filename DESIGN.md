...
- **v0.5 는 얇은 vertical slice 로 시작한다** — Listeners / Pools & Backends(드레인) /
  ...
  지금 열린 것은 Plan·Impact 와 Listeners 두 장이다. 경로는 `/` 와 `/listeners`
  이고, `serveGui` 의 확장자 없는 폴백이 같은 `index.html` 을 낸다. 세 번째
  (Pools) 가 붙기 전에는 Kit 을 깔지 않는다.
  ...
  소켓 충돌은 검증기가 막는다 — 화면이 다시 재지 않는다. 폴링하지 않는다.
