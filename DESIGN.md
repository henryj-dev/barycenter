...
`route create` 는 HTTP 호스트 → 풀 proxy 이거나 URL redirect 다. websocket 은 기본이 끈다.
redirect 는 `to` 와 301·302·307·308 이다. 기본 302. pool 을 안 붙인다.
reject·패스스루 라우트는 아직이다. 드레인은 S2 다.
...
| **v0.4** CLI | export/import ✅ · 나뉜 changeset 단계 ✅ (`changeset new\|patch\|plan`, `commit --plan`, `apply --plan`). listener create ✅. 풀 create ✅. HTTP proxy·redirect 라우트 create ✅. reject·패스스루 라우트는 아직 | 같은 매니페스트를 두 번 import 해도 결과가 같다. `apply --plan` 은 changeset 을 안 연다 |
