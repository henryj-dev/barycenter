...
> **아직 세대 보존이 이걸 안 읽는다.** 상한이 걸린 배포에서도 GC 는 여전히 숫자 상한만
> 본다. 시간 기반 보존은 그다음 회차의 일이고, 그때 이 필드가 전제가 된다.
...
#### HTTP/3 — 무엇이 다른가 (S20 이 열어야 한다)

`nginx -t` 는 통과하고 **소켓도 실제로 열린다**:

```
listen 19820 ssl;              → tcp  0.0.0.0:19820 LISTEN
listen 19820 quic reuseport;   → udp  0.0.0.0:19820
Alt-Svc: h3=":19820"; ma=86400 → 응답에 나간다
```

**그런데 h3 로 실제 붙는 것은 아직 못 쟀다.** 엔진 이미지의 `curl` 이 HTTP/3 를 모른다
(`nghttp2` 만 있고 ngtcp2/quiche 가 없다). *"설정이 선다"* 와 *"서빙된다"* 는 다르고,
이 저장소는 그 차이로 여러 번 물렸다 — **h3 클라이언트를 구하는 것이 S20 의 첫 과제다.**
