...
cat <<'GATES'
 ...
 ⚠️ S12 는 **게이트 부하 아래에서 간헐로 떨어진다** (2026-08-24 확인). 단독으로는 세 번
     연속 초록인데 전체 게이트 안에서는 `S12.sweep`(#28 phase 가 빈다)과 `S12.lock`
     (죽은 주인의 락 회수)이 번갈아 떨어진다. **이건 S14 를 게이트 밖에 둔 것보다 나쁘다**
     — 게이트 안의 흔들림은 빨강을 무시하는 법을 가르친다.
     **서명을 하나 붙잡았다** (2026-08-24): 복구 러너가 `terminalState: 'failed'` 로
     죽어 있었다 — `DpRejection('terminal')` 이다. 앞선 복구 시도가 활성화 증거를 예산
     안에 못 보고 `fail` 을 찍었고, 그 뒤 재시도가 원장에 막혔다. 그 회차에도
     `link`·`served` 는 둘 다 gen-2 였다 — **세상은 맞았고 좌표만 뒤에 남았다.**
     프로덕션은 `reconcileConfig` 가 주기적으로 덮지만 이 스파이크는 그 수렴을 안 태운다.

     **가설이지 확정이 아니다** — 서명이 하나뿐이고 재현이 부하 의존이다. 그 사이
     진단을 두 겹으로 넓혔다: probe 가 실패 회차의 로그를 25 줄 남기고(4 줄이라
     예외의 종류가 잘렸다), `verify.sh` 가 표식 줄을 함께 낸다(꼬리 25 줄만 내서
     인라인 진단이 통째로 잘렸다). 다음 재현이 가설을 확인하거나 뒤집는다.
