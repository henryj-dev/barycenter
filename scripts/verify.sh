...
cat <<'GATES'
 ...
 ① lease 가 타입이 아니라 주석이다                          [타입 모양]
    FsEffects.signalReload 는 lease 를 확인한 뒤 주입된 reload() 를 await 한다.
    그 안에서 fence 가 끼어들면 신호는 그대로 나간다. 실측: 검사 통과 · HUP 1회.

 ② drive 결박이 ID 두 개뿐이다                              [구현 품질]
    같은 operationId 로 gen-A 뒤 gen-B 를 내면 2차도 activated 인데
    게시된 것은 gen-A 다. **조용한 거짓 성공.**

 ③ 실행권이 없어도 lease 가 유효하다                        [구현 품질]
    assertOwnership 은 holder 가 undefined 면 통과시킨다. abortConfig 뒤에도
    멈춰 있던 부작용이 착지한다.

 ⑤ 부분 활성화 뒤 abort 가 중단된다                          [구현 품질]
    첫 평면이 terminal 이라 던지고, 나머지 예약과 실행권이 남는다.

 ④ 불투명 저장소가 불투명하지 않다                          [타입 모양]
    {version} 만 보관하는 정직한 구현은 두 번째 fence 에서 TypeError.
    .d.ts 도 여전히 AgentState 를 노출한다.
