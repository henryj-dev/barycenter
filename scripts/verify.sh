...
cat <<'GATES'
 ...
 계측기 둘이 생겼다.
   · 표면   `scripts/surface.mjs` — 76 심볼 폐포 해시. 동결 카운터가 센다.
 ...
 ─── A 를 막는 것이 이제 무엇인가 (15차 시점) ────────────────────────

 **미해소(❌) 항목이 없다.** 남은 것은 둘뿐이다.
  ...
  2. **▲ 로 남은 잔여물** — 셋 다 "못 막는다" 가 아니라 **"막는 대신 덮는다"** 로
     답한 것이고, 각각 반례가 걸려 있다.
       · 확인과 부작용 **사이의 await** (9차 ①) — 수렴이 덮는다
       · 펜싱 TOCTOU (7차 ②) — 늦은 착지를 관측하고 되돌린다
       · 예산 초과가 부분 커밋을 숨김 (13차 ③) — 여섯 번 재현 실패, 방어적으로 고침

 → A(타입·DP ABI): **No-Go** — 다만 이유가 "구멍이 있다" 가 아니라
                    **"표면이 안 변한 회차가 아직 없다"** 로 바뀌었다.
