...
cat <<'GATES'
 ...
 ① lease 논증        ▲ lease 는 창을 좁히는 장치로 남는다. **타입이 강제하지 않는다** —
                      주입된 콜백이 안에서 await 하면 신호는 나간다. 그건 수렴이 덮는다.
 ...
 ① Effects 에 펜싱이 없다   ❌ **논증이 틀렸다** (9차 검수). "assertValid 와 부작용
                             사이에 await 가 없다" 는 **구현자에게 부탁한 주석**이지
                             타입이 강제하는 계약이 아니다. 주입된 reload 가 안에서
                             먼저 await 하면 검사 뒤에 신호가 나간다 — 재현했다.
