...
2026-08-13~14 에 **네 번** 터졌다 — 한 커밋이 남의 문서를, 다른 커밋이 남의 소스를
삼켰다(이 훅을 처음 쓴 저장소에서의 실측이다). 배포가 붙은 경로가 섞이면 **CI 가 그대로
...
관여하지 않는다 — 거기서의 머지·커밋은 그 저장소의 규칙을 따른다.
...
# 명령 앞머리의 `cd <경로>` 도 봐야 한다. 2026-08-14 에 `cd <중첩 저장소> && git merge` 가
...
def git(cwd, *args, timeout=15):
    ...
    **매달림은 실패가 아니라 침묵**이라 그 안전판에 안 걸린다.
    ...
    (같은 파일의 `proc_start`·`proc_cmd` 는 처음부터 `timeout=5` 를 갖고 있었다.
     여기만 없었다 — 2026-08-17, 다른 저장소의 검수가 남긴 「이름이 아니라 API 로
     훑어라」를 우리 훅에 대 보다 나왔다.)
    """
    try:
...
def protected_main():
    ...
    참이 된다. 하위 디렉터리에 다른 저장소를 체크아웃해 두면 그것들이 전부 걸린다 —
    남의 저장소를 우리 규칙으로 막는 것이고, 2026-08-14 에 실제로 그런 머지가 막혔다.
