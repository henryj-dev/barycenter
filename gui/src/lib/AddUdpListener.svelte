<script lang="ts">
  ...
  let { pools, editing, add }: {
    pools: string[];
    editing: boolean;
    add: (key: string, bind: string, port: number, pool: string, preset: UdpPreset) => void;
  } = $props();
  ...
  let port = $state('53');
  let pool = $state('');
  let preset = $state<UdpPreset>('dns');

  const submit = (): void => {
    add(key.trim(), bind.trim(), Number(port), pool, preset);
  };
</script>
...
<form class="add" onsubmit={(e) => { e.preventDefault(); submit(); }}>
  ...
  <select bind:value={preset} disabled={editing}>
    <option value="dns">dns</option>
    <option value="wireguard">wireguard</option>
    <option value="game_generic">game</option>
    <option value="custom">custom</option>
  </select>
  <select bind:value={pool} disabled={editing || pools.length === 0}>
    <option value="">UDP 풀</option>
    {#each pools as p (p)}
      <option value={p}>{p}</option>
    {/each}
  </select>
  <button type="submit" disabled={editing || pools.length === 0}>UDP 포트를 연다</button>
</form>

<style>
  .add {
    display: grid;
    grid-template-columns: 6rem 7rem 4.5rem 6.5rem 7rem auto;
    gap: 0.4rem;
    margin-top: 0.8rem;
    padding-top: 0.8rem;
