...
<form class="add" onsubmit={(e) => { e.preventDefault(); submit(); }}>
  ...
  <input bind:value={hashKey} placeholder="hashKey" autocomplete="off" disabled={editing} />
  ...
  <button type="submit" disabled={editing}>hash 풀을 연다</button>
</form>

<style>
  .add {
    display: grid;
    grid-template-columns: 6rem 5rem 8rem 7rem 1fr 4.5rem auto;
    gap: 0.4rem;
    margin-top: 0.8rem;
    padding-top: 0.8rem;
