...
> ✅ **끝났다.** 아래 `- [ ]` 는 착수 절차의 기록이다 — 남은 일이 아니다.
...
> ✅ **완료됐다 (2026-08-22).** 아래 `☐` 는 **실행 당시의 절차 기록**이지 진행
> 상태가 아니다 — 결과는 맨 위 「실행 기록」이 정본이다.
...
### ☑ 닫혔다 (2026-08-23) — 사람이 해제를 결정했다

셋 다 들어왔다. `git merge origin/audit-w3-surface` → 표면 112 심볼 · 카운터 0 · 미선언.

**투두에 적힌 처방은 틀렸다.** *"`surface.mjs --write` 한 번이면 끝난다"* 고 적어 뒀는데
돌려 보면 그대로 거부한다 — `--write` 는 동결 상태를 하드 차단하고 **해제 플래그가
없었다.** 남은 길은 `SURFACE.txt` 를 손으로 고치는 것뿐이었고, 그 파일 머리에는
*"손으로 고치지 않는다"* 가 적혀 있다.

**막다른 길은 규칙을 지키게 하는 게 아니라 어기게 만든다.** 그래서 길을 내되 싸지 않게
냈다 (`e5a38c7`):

  · `--write` 는 동결 상태에서 **여전히** 하드 차단이다. 해제는 별도의 행위다 —
    계약을 옮기다 실수로 동결까지 함께 풀리면 안 된다.
  · `--unfreeze "<근거>"` 가 **근거를 요구한다.** 근거 없는 해제는 다음 사람이 "왜
    풀렸지" 를 물을 때 답이 없고, 답이 없으면 쌓아 온 회차가 뜻을 잃는다.
  · 근거는 `SURFACE.txt` 머리에 남고 `--write`·`--round`·`--freeze` 를 지나서도
    **안 지워진다.** "한 번 풀린 적 있다" 는 다음 사람이 이 숫자를 얼마나 믿을지 정할
    때 쓰는 사실이다.

그리고 이 회차가 드러낸 것 하나 더: **동결 게이트를 재던 테스트가 게이트의 현재 상태에
묶여 있었다.** 라이브 `SURFACE.txt` 를 복사해 `mutate('동결 카운터: 3', ...)` 로 썼으므로
실제로 풀리자 5 건이 한꺼번에 빨개졌다. 이제 본문만 라이브에서 가져오고 머리는 테스트가
찍는다.

**대가는 그대로다** — 재동결까지 3 회차를 다시 쌓아야 한다. 그게 이 결정의 값이다.
...
4-1. **S12 간헐 실패 — 재현했고, 성격이 특정됐다.**

   앞서 두 번 봤지만 실패한 실행의 전체 출력을 못 잡았다(PASS/FAIL 줄만 걸러 봤다).
   러너는 **실패 시 진단을 이미 찍는다** — 내가 그걸 버린 것이다. 다시 잡았다.

   **재현 방법:** `spike/s12/run.sh` 를 **동시에 둘씩** 돌려 스스로 부하를 만든다.
   8 회(4 라운드 × 2)에서 2 회 실패했다.

   **관측한 것** (두 실패의 서명이 같다):

   ```
   #25 reload:after                  → phase=(3회) link=gen-2 served=gen-2
                                       next=(3회) next_link=gen-2 next_served=gen-2
   #34 journal:reload_observed:before → (같은 모양)
   ```

   · 지점이 **서로 다르다**(#25 · #34).
   · 복구 러너가 `RejectionError(terminalState:'failed')` 로 죽는다.
   · 그런데 **세계는 이미 gen-2 로 수렴해 있다** — `link` 와 `served` 가 둘 다 gen-2 다.
   · 후속 오퍼레이션(gen-3)이 3 회 다 출력을 못 내고 막힌다.

   **이것이 무엇인가 — 저장소가 이미 답을 적어 뒀다.** `spike/s12/runner.mjs` 의 주석:

   > 처음엔 `{attempts:3, intervalMs:50}` 으로 빠르게 돌렸는데, 그건 실제보다 17 배
   > 빡빡한 예산이라 **활성화 증거를 기다리다 실패로 확정하는 회차가 생겼다.** 스윕이
   > 회차마다 다른 지점을 실패로 지목했고(#2·#15·#24 → #10), 그 비결정성이 단서였다 —
   > **고정된 로직 결함이면 같은 지점이 나온다.**
   > 계측기를 실제와 다르게 맞추면 없는 결함을 만들어 낸다.

   내가 만든 부하가 프로덕션 폴 예산(`DEFAULT_POLL` 25×100ms)을 **상대적으로** 그때와
   같은 상태로 만들었다. 즉 **내 재현 방법 자체가 그 주석이 경고한 함정이었다** — 지점이
   흩어지는 것이 그 판별이고, 실제로 흩어졌다.

   **그러므로 이것은 고정된 로직 결함의 증거가 아니다.** "재실행하면 초록" 으로 닫은
   것이 아니라, 무엇인지 알고 닫는다.

   **남는 진짜 물음은 따로 있고, 이미 목록에 있다.** 활성화를 *관측하지 못한 것*과
   *일어나지 않은 것*은 다른데 러너는 둘을 `failed` 로 합친다(S7 이 "세상은 이미 수렴해
   있다" 를 실측했다). 그건 `verify.sh` 의 ▲ 목록에 **"예산 초과가 부분 커밋을 숨김
   (13차 ③)"** 으로 이미 서 있는 항목이고, 수렴 판정을 건드려야 하므로 이 회차의 일이
   아니다. 이번 관측은 그 항목에 **재현 가능한 무대**를 하나 붙인 것이다.
