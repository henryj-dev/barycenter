...
## W3 · 표면 회차 — **구현은 끝났고 동결 해제 결정만 남았다**
...
- [ ] ☐ **W4-3 · S-08b — 적힌 장애물이 사실이 아니다.** 조사해서 고친다.
      · 원래 기록: *"세대에 비밀을 구우면 digest 가 비밀의 함수가 되어 결정적 렌더 계약과
        충돌한다."* **그 충돌은 안 생긴다** — `render_digest` 는 `conf` 만의 함수이고
        admin 조각은 `include admin/*.conf` 글롭으로 들어온다(`render.ts:279` 가 왜
        그렇게 했는지 적어 뒀다). admin conf 를 바꿔도 digest 는 안 움직이고 세대 전환도
        안 붙는다.
      · **그리고 비밀이 아예 필요 없다.** 답은 유닉스 도메인 소켓이다 — `listen unix:` 로
        옮기면 접근 통제를 OS 가 지고, conf 에는 경로 리터럴만 남아 결정성이 그대로다.
        비-root 전환(S-10b)이 끝났으므로 소켓을 그 사용자 소유 0600 으로 둘 수 있다.
      · **진짜 비용은 전송 리팩터다.** `fetch('http://127.0.0.1:port/...')` 가 6 곳
        (barycenterd 둘 · acme-runner · drain · plane)이고, `fetch` 호환 유닉스 클라이언트를
        하나 만들면 테스트 주입부는 그대로 둘 수 있지만 e2e 5 · store 4 · conformance 1
        스위트가 admin 포트를 직접 만진다. 포트가 사라지면 W2 의 `adminPortConflicts`
        (S-08a)도 같이 정리 대상이 된다. **약 15 파일 · 멤버십 평면 · e2e 가 유일한 그물.**
      · 실제 노출도 확인했다: compose 배포에서는 admin 이 컨테이너 netns 안이라 안 열려
        있다. 그러나 §11.3 이 권장하는 **v1 배포가 hostNetwork** 이고, 거기서는 같은
        호스트의 아무 프로세스나 슬롯을 다시 쓸 수 있다. 이론이 아니다.
      · 남기는 이유는 결정이 아니라 **규모**다. 멤버십 평면을 긴 세션 끝에 손대는 것이
        이 저장소가 반복해서 기록한 실패 모양이다. 설계는 위에 있으니 다음 회차는
        다시 도출하지 않아도 된다.
- [ ] ☐ **S-06 나머지 — JWKS** 엔드포인트 + 키 회전. RS256 자체는 열렸고 키는 파일로
      준다. 회전을 자동화할지, 한다면 캐시·실패 정책을 어떻게 둘지는 **진짜 결정**이다
...
4. **S12 스파이크가 이번엔 깨끗했다.** 지난 회차에 FAIL=1 로 기록해 둔 간헐 실패가
...
| W1 | 4 | 1 | 2 | — | — | ☑ 완료 |
| W2 | 7 | — | 1 | 3 | 2 | ☑ 완료 |
| W3 | 3 결함 | — | — | 2 | 1 | ☑ 구현 완료 · `audit-w3-surface` (동결 해제 대기) |
| W4 | 9 | — | 3 | 3 | — | ☑ 5 닫힘 · ☐ 3 남음 (둘은 규모, 하나는 결정) |
...
`S-10b`(비-root 컨테이너) · `B-08` · 제안 #10(부분)
...
| `S-08b` | **적힌 장애물이 틀렸다** — 렌더 충돌은 안 생긴다. 진짜 비용은 15 파일 전송 리팩터 |
