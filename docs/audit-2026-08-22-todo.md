...
- [ ] ☐ **W4-3 · S-08b** admin 엔드포인트 인증. 세대에 비밀을 구우면 digest 가 비밀의
      ...
      (스케치 하나: conf 에는 변수 **이름**만 넣고 `init_by_lua` 가 `os.getenv` 로 읽는다.
      digest 는 이름의 함수라 결정적이지만, 엔진 프로세스에 비밀을 넘기는 배포 경로가
      바뀐다. 검증 안 했다.)
...
- [ ] ☐ **S-06 나머지** JWKS 엔드포인트 + 키 회전, PKCE, nonce. RS256 자체는 열렸고
      키는 파일로 준다. 회전을 자동화할지, 한다면 캐시·실패 정책을 어떻게 둘지는 결정이다
...
| 커밋 | 항목 | 핀 |
|---|---|---|
| `11cf40e` | DB 보존 (B-08 · 제안#10) | 3 · store |
| `15c98eb` | OIDC RS256 도달 · `azp` · 역할 클레임 (S-06 · S-07) | 3 · unit |
| `7a16f32` | 제어 API 서버 TLS (S-05b) | 3 · unit |
...
W4 에서 추가: `S-05b`(서버 TLS) · `S-06`(RS256 도달) · `S-07` · `B-08` · 제안 #10(부분)
...
| `S-08b`·`S-10b` | admin 인증(결정적 렌더와 충돌)·비-root 컨테이너 — 설계가 먼저 |
