...
**안 고친다 — 이 회차에서는.** 게이트에 넣는 것은 판단이 필요하다: 다섯 배포판 컨테이너는
`verify.sh` 의 임계 경로를 크게 늘리고(#15 가 873초를 254초로 줄인 직후다), nightly 로
보내면 "머지 뒤에 안다" 가 된다. **어디에 넣을지가 결정 사항**이라 코드보다 결정이 먼저다.
그 결정을 이 회차의 산출물로 남긴다.
