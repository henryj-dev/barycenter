...
아래 항목 중 R2~R4는 이번 회차에 처리했고, R1의 KMS/Vault 드라이버만 실제 미구현
로드맵이다. 코드가 스스로 범위를 선언한 잔여 위험은 숨기지 않고 운영 계약으로 남긴다.

### ☑ R-1 · KMS/Vault SecretStore 드라이버 `[L]` — 드라이버는 로드맵, 자세 표시는 처리
...
현재 1~6은 각각 구현 또는 명시적 잔여 위험으로 처리됐다. R-1의 KMS/Vault 드라이버만
제품 범위를 넓히는 후속 과제다.
