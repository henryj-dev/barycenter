...
V1~V4와 D-1/D-2는 구현·검증을 마쳤다. 현재 기준 커밋은 `e4ec324` · `6c3bf2c` ·
`75ee736` · `6afbdad` · `5da0ca7`이며, 후속 운영 문서와 SecretStore 자세 로그는
`89d1053`에 반영했다. 아래의 미체크 항목 중 이 커밋들로 충족된 줄은 역사적 작업 목록으로 남아
있으며, 최종 게이트에서 다시 확인한다.
...
### ☑ D-1 · 평문 노출을 어떻게 다룰 것인가 `[L]` (문서·구현 완료)
...
### ☑ D-2 · `configTest` 실행 실패를 무엇으로 볼 것인가 `[L]` (문서·구현 완료)
...
아래 항목 중 R2~R4는 이번 회차에 처리했다. R1은 **2026-08-28 에 `PgSecretStore` 로
닫혔다** — 남은 것은 KMS/Vault 라는 *특정 드라이버*이지 "암호화 저장소가 없다" 가
아니다. 코드가 스스로 범위를 선언한 잔여 위험은 숨기지 않고 운영 계약으로 남긴다.

### ☑ R-1 · 암호화 SecretStore 드라이버 `[L]` — **닫혔다 (2026-08-28)**

> **`PgSecretStore` 로 닫았다** (§4.8.1). 벤더 API 가 아니라 **이미 있는 PG** 로 채웠다 —
> 봉투 암호화(자료당 DEK · KEK 는 DB 밖)라 §4.8 의 평문 금지 조항을 지키고, 덤프 하나로는
> 키가 안 나온다. 아래 「남는 위험」 두 줄이 그래서 닫힌다.
>
> **KMS·Vault 는 여전히 로드맵이다.** 같은 인터페이스 뒤 세 번째 드라이버 자리이고,
> `pg` 가 그 자리를 막지 않는다 — KEK 를 KMS 에서 받는 것이 다음 단계다.
>
> 대가가 하나 생겼다: **KEK 를 잃으면 자료를 못 연다.** 백업 세 종의 3번이 바뀌었다
> (`docs/runbook-spof.md`).
>
> 딸려온 것: `SecretStore` 의 여섯 메서드가 async 가 됐다(`facts` 만 동기 — 검증기와
> plan 이 동기 경로다). GC 정책은 `partitionForSweep` 한 자리로 모았고 스캐너만 둘이다.
> 표면 A 는 **안 움직였다** — `SecretStore` 는 공개 표면에 없다.
...
- [x] ~~남는 위험은 그대로다 — 호스트 침해 시 즉시 노출, 백업·스냅샷·디스크 복제본 잔존~~
      → `BARY_SECRET_BACKEND=pg` 에서 둘 다 닫힌다. `fs` 를 고른 배포에는 그대로 남고,
      그것이 기본이므로 **기동 자세 로그가 계속 그 사실을 말한다.**
- [x] 그때까지 할 수 있는 것: **기동 시 저장 방식과 파일 권한을 검사해 로그에 드러낸다.**
      드라이버보다 훨씬 싸고, 「암호화된 줄 알았다」를 막는다

### ☑ R-2 · GUI 인증을 HttpOnly 세션으로 `[L]`

`gui/src/lib/desk.svelte.ts` · `gui/src/routes/login/+page.svelte` — OIDC 교환은 데몬의
`bary_login`/`bary_session` HttpOnly 쿠키를 사용한다. GUI에는 OIDC 토큰·PKCE 검증값을
저장하지 않는다. 수동 Bearer 입력은 새로고침하면 사라지는 메모리 값이다.

- [x] 데몬 내부 세션 + HttpOnly/Secure/SameSite 쿠키로 처리했다. GUI를 정적으로 서빙하는
      구조(`api.guiRoot`)는 유지하고, `bary_login`에 PKCE 자료를, `bary_session`에 검증된
      principal을 묶는다.

세션은 인스턴스 메모리이며 재시작·다중 인스턴스 사이에 공유하지 않는다. 다중 인스턴스가
필요해지는 배포의 외부 세션 저장소·폐기 계약은 별도 후속 결정이다.

### ☑ R-3 · SecretStore 미설정 시 인증서 자료 업로드 불가 `[S]` — 문서화 처리

`src/api/server.ts:737` — SecretStore 가 없으면 `POST /api/v1/certificates/material` 이
`501 no_secret_store` 다. 선택형 의존성 설계인 것은 확인됐다.

- [x] 배포 구성에서 「인증서 업로드는 항상 된다」고 기대하면 공백이 된다 →
      **문서로 충분한지** 판단한다. 코드 수정이 아닐 가능성이 높다

`DESIGN.md §8.1`과 `STATUS.md`에 `501 no_secret_store`와 선택형 의존성을 적었다.

### ☑ R-4 · DNS-01 provider 범위 `[M]` — 파일 훅 계약 문서화 처리

`src/control/dns01.ts` (전체 40 줄) — `FileDns01` 뿐이다. 헤더가 *"벤더 API 를 제품
계약으로 얼리지 않는다. 파일이 정본이다"* 라고 적었으므로 **의도된 범위**다.

- [x] 외부 DNS 자동화가 필요한 환경에서는 배포가 훅을 제공해야 한다 →
      그 훅의 계약을 문서에 적을지, 범용 provider 인터페이스를 열지 정한다

범용 벤더 인터페이스는 열지 않고, `DESIGN.md §8.1`에 파일 경로·파일 내용·cleanup 및
외부 `nsupdate`/DNS 훅의 소유권을 계약으로 적었다.
...
| V4 | 2 | 레이트리밋 `[중간]` | — | 불변 |
| R | 4 | R1 드라이버는 로드맵, R2~R4는 처리·문서화 | — | R2의 세션은 인스턴스 메모리 |

**보고서 우선순위와의 대응** — 1 → V3 · 2 → V2 · 3 → V4 · 4 → V1 · 5 → R-1 · 6 → R-2.
현재 1~6은 각각 구현 또는 명시적 잔여 위험으로 처리됐다. R-1 은 2026-08-28 에
`PgSecretStore` 로 닫혔고, **KMS/Vault 드라이버**만 제품 범위를 넓히는 후속 과제로 남는다.
