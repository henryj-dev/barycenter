...
`75ee736` · `6afbdad`이며, 후속 운영 문서와 SecretStore 자세 로그는 `89d1053`에
반영했다. 아래의 미체크 항목 중 이 커밋들로 충족된 줄은 역사적 작업 목록으로 남아
...
### ☐ R-2 · GUI 인증을 HttpOnly 세션으로 `[L]` — 구조적 로드맵으로 명시
...
이번 회차에는 토큰 교환 BFF·세션 저장소를 임의로 추가하지 않았다. 현재 구조와 잔여 위험,
필요한 선행 결정(세션 공유·폐기·쿠키 계약)은 `DESIGN.md`와 `STATUS.md`에 명시했다.
