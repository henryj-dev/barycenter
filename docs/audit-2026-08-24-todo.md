...
### ☐ W3-5 · 소크 관측 — `agent.json` 이 상한 없이 자란다 `[M]`

> W2 마무리의 소크가 낸 숫자다. 6 분 · 설정 변경 37 회에 `agentStateKb` 가
> **6 → 147** 로 **평탄해지지 않고** 자랐다(오퍼레이션당 ~4 KB). 같은 실행에서
> `generations`·`generationKb`·`rssMb` 는 전부 평탄해졌다 — 보존 정책이 서는 자리와
> 안 서는 자리가 갈린다.

- [ ] 무엇이 자라는지 먼저 짚는다. `prune` 이 `evidence`·`terminal`·`completed` 셋을
      자르는데도 자란다 — **자르는 목록에 없는 것**이 있다는 뜻이다
