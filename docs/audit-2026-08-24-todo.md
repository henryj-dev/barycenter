...
- [ ] `tests/unit/audit-reachable-methods.test.ts` — 호출자 없는 public 메서드를 가진
      픽스처에서 게이트가 그것을 잡는가
- [ ] **빨강 확인**
- [ ] `scripts/reachable.mjs` — export 된 클래스의 public 메서드도 센다. 소비자 스캔은
      지금과 같다(`src/` + `gui/src/`). 예외가 필요하면 `ALLOW` 에 **이유와 함께**
- [ ] 걸려 나온 것을 전부 본다 — 이 회차에 `versions()` 하나를 손으로 찾았고,
      게이트가 더 찾으면 그것이 이 항목의 값이다
- [ ] `npm run verify:quick`
- [ ] 커밋 — `Pinned-by: tests/unit/audit-reachable-methods.test.ts -t "호출자 없는 public 메서드를 잡는다"`
...
**29 개 전부 배정됐다.** D20 은 검수 문서 §4 에 적힌 대로 직전 회차(`b8c7eb2`)가
