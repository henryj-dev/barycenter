...
- [x] `./scripts/verify.sh` 전체 — 20/20 초록
- [x] 소크 6 분 — `npm run soak -- --minutes 6`

**소크가 둘을 냈고 둘 다 값이 있었다.**

1. **트래픽 실패 9.4%** (5712 건 중 536). 그때는 원인을 몰랐는데, W3-0 이 무대를
   세우고 나서 **N5(페일오버 없음)** 였다는 것이 드러났다 — 완전히 다른 하네스가
   같은 것을 짚은 셈이다
2. `agentStateKb` 6 → 147. **이건 내가 잘못 읽었다** — 상한은 있다(W3-5 의 정정)

> **소크가 게이트 밖이라는 것도 이 회차에 대가를 치렀다.** `verify.sh` 에 없으므로
> N4-b(`scripts/soak.mjs` 가 워크트리에서 안 도는 것)가 아무 신호도 안 냈고, 손으로
> 돌리려다 발견했다. 게이트에 넣을지는 다음 회차의 물음이다 — 6 분이 회차마다
> 붙는 값인지 아직 모른다
...
### ☑ W3-1(원문) · D3 — `in:` 누수를 막는다 `[M]` — **위 항목에 흡수됐다**

> 원문을 지우지 않고 남긴다. **무엇을 계획했고 실물이 그것을 어떻게 바꿨는지**가
> 이 문서의 값이고, 지우면 그 기록이 사라진다.
>
> 계획은 맞았다 — `ngx.ctx.bary_peers` 에 쌓고 log 단계가 전부 내린다, 양 평면 다.
> **틀린 것은 전제였다**: 그 누수는 재시도가 있어야 나는데 재시도가 없었다(N5).
> 그래서 이 항목만 고쳤으면 **아무 일도 안 하는 다섯 줄**이 들어갔을 것이다.

- [x] 전부 위 W3-1 에서 한 커밋으로 처리했다
...
- [x] `./scripts/verify.sh` 전체 (도커 포함)
- [x] **골든 재생성은 할 것이 없었다.** 이 저장소의 「골든」은 스냅샷 파일이 아니라
      **실물 nginx 거동 테스트**다 — `tests/golden/__snapshots__` 이 없고
      `toMatchSnapshot` 도 안 쓴다. 렌더가 바뀌면 그 테스트가 직접 빨개지고, 실제로
      W3-1 이 그렇게 잡혔다
- [x] `STATUS.md` — 아래 W4 마무리에서 함께

> **투두가 없는 절차를 적어 뒀던 자리다.** 「골든 재생성」은 스냅샷 방식일 때의 말이고,
> 이 저장소는 그 방식이 아니다. 확인하지 않고 적으면 다음 사람이 없는 단계를 찾는다.
...
### ☑ W4-9 · 인프라 flake 를 세는 자리가 없다 `[S]`

이 회차에 **게이트가 우리 코드와 무관하게 빨간 적이 넷**이다:

| 무엇 | 원인 | 어떻게 알았나 |
|---|---|---|
| 골든 `ciphers` | 파일 병렬로 부하가 몰려 TLS 협상이 늦었다 | 단독 재실행이 초록 |
| 스파이크 S18 | Pebble 의 nonce 20% 거부 × 요청 수백 개 | 단독 재실행이 초록 |
| `store` `config-store` | PG 컨테이너 `read ECONNRESET` (25 테스트 전부 skipped) | 단독 재실행이 초록 |
| 전체 게이트 3 회 | 외부에서 종료됨 (우리 것이 아니다) | 로그가 중간에 끊김 |

앞의 셋은 고쳤거나(골든 직렬화 · S18 재시도 예산) 한 번뿐이다(`store`). **문제는
그것을 세는 자리가 없다는 것**이다 — 「단독으로 돌리면 초록」이 세 번 반복됐는데, 그
사실 자체는 어디에도 안 남는다.

- [x] **㉠ 을 골랐다** — 재실행하고 기록하되 **판정은 안 바꾼다**
- [x] `scripts/lib/flake.sh` — `bary_run_twice` · `bary_record_flake`
- [x] `scripts/lib/test-flake.py` — 14 케이스. **실제 셸에서 소스해 실물 명령으로** 돈다
- [x] `scripts/verify.sh` — `run()` 이 실패하면 한 번 더 돌린다.
      두 번째가 초록이면 `FLAKE` 로 적고 `.flakes.jsonl` 에 남긴다
- [x] **계수기 자신도 게이트에 넣었다** (`흔들림 계수기` 줄) — 세는 쪽이 틀리면
      그 숫자가 거짓이 된다
- [x] `./scripts/verify.sh --quick` 10/10 초록

**왜 재실행인가.** 「흔들림」인지 「진짜 빨강」인지 알 수 있는 순간이 **그때뿐**이다 —
같은 컨테이너·같은 부하에서 바로 다시 돌려야 알 수 있고, 하루 뒤에는 아무도 재현
못 한다. 기록만 하는 길(㉡)은 이미 화면에 보이는 것을 파일로 옮길 뿐이라 flake 와
진짜를 못 가른다.

> ⚠️ **재실행은 위험한 도구다.** 이 저장소가 적어 뒀다 — *"단독으로 돌리면 초록이라
> 원인을 찾기 전에 「가끔 깨진다」로 넘어가기 쉽다"*, *"간헐적으로 깨지는 게이트는
> 없느니만 못하다"*.
>
> **그래서 재실행이 판정을 안 바꾼다.** 두 번째가 초록이어도 그 스위트는 실패로 세고
> exit 코드는 빨갛다. 재실행이 사는 이유는 **「흔들렸다」를 「깨졌다」와 구분해서
> 적기 위해서**이지 통과시키기 위해서가 아니다. 자동 재시도로 초록을 만드는 길은
> **일부러 안 만들었다** — 있으면 언젠가 쓰인다(`rejectUnauthorized` 를 끄는 옵션을
> 안 만든 것과 같은 판단이다).

**재현물이 셋을 함께 잰다.** 「흔들림을 잡는가」만 재면 **전부 흔들림이라고 답하는
구현**도 통과한다 — 초록은 재실행을 안 하고, 흔들림은 판정이 여전히 빨갛고, 진짜
빨강은 기록에 안 들어간다. 그리고 **진단은 실패한 실행의 출력**이어야 한다: 초록인
두 번째 출력에는 단서가 없다.

**장부는 커밋 안 한다.** 「이 기계에서 게이트가 얼마나 흔들렸나」는 기계마다 다르다 —
공유해야 할 값이면 CI 가 모을 일이다.

### ☑ W4 마무리

> **투두에 없던 절이다.** W0~W3 에는 마무리가 있는데 W4 만 없었다 — 「결정이 먼저」인
> 블록이라 항목마다 게이트를 돌린다고 봤던 것 같지만, **표면이 두 축 다 움직인 블록**
> 이라 오히려 마무리가 더 필요하다.

- [x] 전체 게이트 — **스위트별로 나눠 돌렸다** (아래 근거)
- [x] `node scripts/surface.mjs --check` — **표면 그대로** (117 심볼)
- [x] `node scripts/freeze-b.mjs --check` — **`ok B freeze 46 routes`**
- [x] `STATUS.md` 를 실측으로 맞췄다

**전체 게이트를 스위트별로 나눠 돌렸다.** `./scripts/verify.sh` 가 **세 번 연속 외부에서
종료**됐고(우리 것이 아니다) 그때마다 20~30 분이 사라졌다. 나눠 돌리면 종료가 한 스위트만
날리고, **증거는 같다** — 같은 명령을 같은 트리에서 돌린 결과이기 때문이다. 다만
`verify.sh` 의 표가 한 번에 안 나오므로 여기 적어 둔다:

| 스위트 | 결과 |
|---|---|
| typecheck · 표면 · 도달성 · node핀 · 재현물핀 · 훅 둘 | 전부 초록 |
| unit | **948** |
| conformance | **479** |
| 모델 | 13 |
| build (dist·gui) | 초록 |
| **진입점 퍼미션** (이 회차 신설) | 16 |
| store (실물 PG) | **234** |
| golden (nginx -t) | **71** (14 파일 · 직렬) |
| e2e (실제 nginx) | **70** (9 파일) |
| engine facts | PASS=76 FAIL=0 SKIP=1 |
| 스파이크 12 개 | **PASS=100 FAIL=0** SKIP=2 |

> **`store` 가 한 번 빨갰다** — `config-store.test.ts` 가 `read ECONNRESET` 로 25 개
> 전부 skipped. 단독 재실행은 25/25 초록이다. **PG 컨테이너 문제이고 우리 코드가
> 아니다** — 이 회차의 인프라 flake 셋째다(→ W4-9).

## CI · 게이트가 리눅스에서 다르게 답한다

> **투두에 없던 블록이다.** W4 를 닫고 「남은 것 없음」을 선언한 뒤 CI 를 보니
> `verify` 워크플로가 **빨갛게 셋 연속**이었다. 로컬 게이트는 그 내내 초록이었다.
>
> 이 회차가 세 번 적은 문장이 여기서 네 번째로 맞는다 — **목록에 없던 것을 게이트가
> 찾았고, 손으로 훑어서는 안 보이는 부류였다.** 다만 이번엔 찾은 게이트가 로컬이 아니라
> **CI** 였다. 그리고 그 차이 자체가 결함의 원인이다: 우리는 게이트를 macOS 에서만
> 돌려 왔고, **macOS 와 리눅스가 다르게 답하는 자리**가 셋 있었다.

### ☑ CI-1 · S15 가 리눅스에서 한 번도 초록인 적이 없다 `[S]`

- [x] 원인 확정 — `mktemp -d` 는 **0700** 이고, 그 디렉토리를 엔진 컨테이너에
      bind-mount 한다. 컨테이너 안 nginx 워커는 `nobody` 라 **통과하지 못한다**
- [x] 기전을 실물로 쟀다 — openresty 이미지 안에서 0700 은 `Permission denied`,
      0755 는 읽힌다
- [x] `spike/lib.sh` 로 빼고 S10 · S15 · S20 셋이 함께 쓴다
- [x] `bary_spike_workdir` 가 700 대신 **755** 를 낸다 (실측 확인)

CI 로그의 문장은 이것이었다:

```
[error] failed to load external Lua file "/w/probe.lua":
        cannot open /w/probe.lua: Permission denied
```

**왜 로컬에서 안 잡혔나.** macOS 의 Docker Desktop 은 bind-mount 를 VM 을 거쳐 넘기면서
소유권·모드를 컨테이너 사용자 쪽으로 바꿔 준다. 그래서 **같은 스크립트가 같은 이미지로
macOS 에서는 초록, 리눅스에서는 빨갛다.** 이 저장소가 적어 둔 *"흉내로 재면 흉내를 재게
된다"* 의 변종이다 — 흉내를 낸 것은 우리가 아니라 개발 기계의 파일시스템이었다.

**셋을 함께 고쳤다.** 같은 패턴이 S10 · S20 에도 있었고 (둘은 게이트 밖이라 안 터졌을
뿐이다), 하나만 고치면 **다음 사람이 리눅스에서 처음 돌릴 때** 같은 자리에 다시 물린다.
이 회차가 N4-b 에서 이미 배운 것이다 — *"목록으로 관리하다 물린 자리가 이미 여럿"*.

**nightly 도 같은 이유로 빨갰다** (`drift` 잡). 한 수정이 둘을 닫는다.

### ☑ CI-2 · 이미지 빌드가 `tsconfig.testing.json` 을 못 찾는다 `[S]`

- [x] `deploy/Dockerfile` 의 `COPY` 에 넣었다
- [x] `.dockerignore` 에 `dist-testing` 을 넣었다 (`dist` 와 같은 이유 — 빌드 컨텍스트만
      불린다)

W4-6 이 `build.sh` 에 `npx tsc -p tsconfig.testing.json` 을 넣었는데 Dockerfile 의 COPY
목록은 안 따라왔다. 이미지 빌드가 `TS5058: The specified path does not exist` 로 죽었다.

**찾기 나쁜 모양이었다** — 저장소에서 도는 `verify` 는 초록이고 `quickstart`(이미지 빌드)
만 빨갛다. 그리고 이것도 **목록 문제다**: W4-7 이 진입점 목록을 없앤 그 회차에, COPY
목록에는 손이 안 갔다.

### ☑ CI-3 · alpine 에 `shasum` 이 없어 lock 감시가 죽어 있었다 `[S]`

- [x] `sha256sum` → `shasum` 순으로 찾고, **둘 다 없으면 빌드를 세운다**

W4-7 이 넣은 gui lock 감시가 `shasum -a 256 … 2>/dev/null` 이었다. alpine(빌드 이미지)
에는 `shasum` 이 없고 `sha256sum` 만 있다 — macOS 는 그 반대다. 그래서 이미지 안에서는
**해시가 늘 빈 문자열**이었고, 빈 문자열끼리 비교하니 항상 같고, lock 이 바뀌어도 다시
안 받는다. **검사 전체가 조용히 죽어 있었다.**

`2>/dev/null` 이 그 사실을 삼켰다. 이 저장소가 세는 자리 그대로다 — *"필드는 있는데
아무도 안 읽는다"*, 그리고 *"도달 불가한 방어는 방어가 아니라 죽은 코드"*. 그래서
빠지는 길을 안 만들고 **없으면 세운다**.

### ☑ CI 마무리

- [x] `./scripts/verify.sh --quick` — 10/10 초록 (unit 948 · conformance 479 · 모델 13)
- [x] `docker build -f deploy/Dockerfile` — 이미지가 선다
- [x] S15 로컬 재실행 — PASS=4 FAIL=0 SKIP=1
- [x] 푸시 후 CI 초록 확인

> **판정은 CI 가 낸다.** CI-1 은 **리눅스에서만 빨간 결함**이라, macOS 에서 아무리
...
| 소크 관측 | Medium | W3-5 (**결함 아님으로 정정**) | M |
| 인프라 flake | Medium | W4-9 | S |
...
**작업 항목 34 개가 전부 닫혔다.** 커밋은 블록당 하나 원칙대로 나뉘어 있고 전부
push 됐다 — `CLAUDE.md` 의 *"push 까지 끝나 있어야 한다"* 가 그 뜻이다.

**목록에 없던 것이 다섯 나왔다** (D21 · D22 · N4 · N4-b · N5). 검수 문서 §10 이
그것들을 정리했고, 셋의 공통점은 **손으로 훑어서는 안 나왔다**는 것이다 — 기계를
켜거나, 실물을 돌리거나, 무대를 세워야 나왔다.

**그리고 이 회차가 스스로 틀린 것이 셋이다:**

1. 소크 관측을 결함으로 읽었다 (§8 정정) — *"실물 실행으로 확인했다"* 를 안 지켰다
2. 재현물이 두 번 가짜였다 (W2-4 의 mtime · W4-4 의 `skipIf`) — **둘 다 핀 게이트가 잡았다**
3. N4 를 고치면서 같은 회차에 `soak.mjs` 를 빠뜨렸다 (N4-b)

셋 다 **게이트가 말해 줘서 알았다.** 그것이 이 저장소가 게이트에 들이는 값의 답이다. D21 · D22 는 **수정 중에 G6 의 게이트가 찾은 것**이라 검수
