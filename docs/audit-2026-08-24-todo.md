...
- [ ] `./scripts/verify.sh` — **전체다.** W0 마무리에서 배운 것: `--quick` 만 돌리면
      도커 스위트가 회차마다 안 재진다. e2e 가 워크트리에서 못 돌던 것(W1-2b)도
      그래서 여기까지 안 드러났다
- [ ] `node scripts/surface.mjs --check` — **W1-2 에서 한 줄 움직였다**(`RejectionKind`).
      기준을 이미 옮겼으므로 여기서는 「그대로」여야 한다
...
- [ ] `tests/e2e/` 다섯 자리의 `데몬이 안 떴다` 를 한 함수로 모은다 —
      `mounts.ts`·`pg-ready.ts` 와 같은 이유다 (다섯에 흩어지면 하나가 뒤처진다)
- [ ] 그 함수가 **로그가 비었을 때** 무엇을 더 싣는지 정한다:
      `docker inspect -f '{{.State.Status}} exit={{.State.ExitCode}}'` 는 최소이고,
      죽은 컨테이너면 `docker logs` 대신 그 종료 코드가 유일한 신호다
- [ ] 재현물 — 일부러 못 뜨게 만든 컨테이너에서 **진단이 비지 않는가**
- [ ] 커밋 — `src/` 를 안 건드리므로 재현물 게이트는 건너뛴다
...
**31 개 전부 배정됐다.** D21 · D22 는 **수정 중에 G6 의 게이트가 찾은 것**이라 검수
