...
### ☐ W4-9 · 인프라 flake 를 세는 자리가 없다 `[S]` — **다음 회차로**
...
- [ ] 게이트가 **재실행으로 초록이 된 스위트**를 세는가. 세면 「가끔 깨진다」가
      인상이 아니라 숫자가 된다
- [ ] ⚠️ **자동 재시도는 답이 아니다.** 그러면 진짜 flake 가 조용해지고, 이 저장소가
      *"간헐적으로 깨지는 게이트는 없느니만 못하다"* 고 적은 그 상태가 된다.
      **세되 초록으로 만들지는 않는 것**이 겨눈 모양이다
...
| 인프라 flake | Medium | **W4-9 — 다음 회차** | S |

**35 개 전부 배정됐다. 하나(W4-9)만 다음 회차로 넘긴다** — 그건 결함이 아니라
**게이트가 자기 흔들림을 안 세는 것**이라, 세는 방법을 정하는 일이 먼저다.
