...
> create listeners, a pool (with its first backend) and an HTTP proxy route
...
- **The API is the only entry point.** The web UI and `bary` CLI are clients of it.
  ...
  backend) and an HTTP proxy route, and stops at commit. Other resource
...
| **v0.4** | `bary` CLI: export/import and changeset steps | ← **done.** listener, pool and HTTP proxy route create exist. Redirect/reject/passthrough routes do not |
