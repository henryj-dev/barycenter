...
> create an HTTP, TCP, UDP or HTTPS listener (commit, not apply); other resource
> commands are still missing. Leader election exists (a PostgreSQL advisory lock
...
- **The API is the only entry point.** The web UI and `bary` CLI are clients of it.
  They are not yet equal: the CLI can create an HTTP, TCP, UDP or HTTPS listener
  and stops at commit. Other resource commands are still missing. Equality is a
...
| **v0.4** | `bary` CLI: export/import and changeset steps | ← **done.** HTTP·TCP·UDP·HTTPS listener create exists. Other resource commands do not |
