...
- **The API is the only entry point.** The web UI and `bary` CLI are clients of it.
  They are not yet equal: the GUI has no `source_ip_hash` form, and the CLI has no
...
| **v0.5** | Web GUI: six screens, SSE, HTTP/TCP/UDP/HTTPS/passthrough writes, cert upload, SNI bind | ← **slice is open.** No Kit, no drain, no source_ip_hash form |
