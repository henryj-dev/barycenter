...
> create an HTTP, TCP, UDP, HTTPS or TLS pass-through listener (commit, not
...
- **The API is the only entry point.** The web UI and `bary` CLI are clients of it.
  They are not yet equal: the CLI can create listeners (HTTP, TCP, UDP, HTTPS,
  TLS pass-through) and stops at commit. Other resource commands are still
  missing. Equality is a
...
| **v0.4** | `bary` CLI: export/import and changeset steps | ← **done.** listener create exists. Pool and route commands do not |
