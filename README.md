...
> create listeners and a pool (round-robin, hash, or source-IP hash, with its
> first backend; commit, not apply); other resource commands are still missing. Leader election exists (a PostgreSQL advisory lock
...
- **The API is the only entry point.** The web UI and `bary` CLI are clients of it.
  They are not yet equal: the CLI can create listeners and a pool (round-robin,
  hash, or source-IP hash, with its first backend) and stops at commit. Other
...
| **v0.4** | `bary` CLI: export/import and changeset steps | ← **done.** listener create and pool create exist. Route commands do not |
