 ...
 * 바뀌면 신호는 그대로 나간다. 표는 "불렀는가" 를 강제할 뿐 "언제 불렀는가" 를 강제하지
 * 못한다. 그건 여전히 규약이다.
 *
 * ✅ **그러나 이제 측정이다** (2026-08-23). `tests/conformance/check-effect-gap.test.ts` 가
 * 두 절을 따로 잰다 — **창이 정말 열리는지**(표를 받고 await 한 뒤 lease 가 무효인데도
 * 쓰기가 나간다)와 **수렴이 정말 덮는지**. 앞의 것을 안 재면 뒤의 것은 아무것도 안
 * 지킨다: 창이 다른 이유로 막혀 있는데 수렴만 초록이면, 그 다른 이유가 사라졌을 때
 * 아무도 모른다. `assertValid` 가 **동기**라는 것도 거기서 잰다 — 비동기였다면 규약을
 * 지킨 구현조차 창을 열게 된다.
 *
