 ...
 *
 * **레거시 창의 방향을 잰다** (49차 E-49-b — 44차에 종단 원장에 했던 것을 여기 안 했다).
 * 구버전 상태를 이 코드에 물리면 `activated` 전환의 **같은 토큰 재시도**가
 * `ok cached=true` 에서 `terminal(activated)` 거부로 바뀐다. **비관·정직 방향**이고
 * 가지치기 후 의미론과 같다 — 반례가 아니다. 미종단 상태의 재시도는 재실행으로 같은
 * 답을 준다(무해).
 *
 * 종단 원장 때와 달리 **레거시 폴백을 안 둔다**: 그쪽은 "포기가 되살아난다" 는 낙관
 * 방향이라 막아야 했지만, 여기는 "재시도가 한 번 더 실행된다" 는 비관이라 안전하다.
 * **방향을 재서 적는 것이 규칙이고**(45차에 안 재서 뒤집혔다), 이번엔 쟀다.
 */
