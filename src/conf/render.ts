 ...
 * 것이 이 평면의 이유이므로 **이건 실제로 다른 계약이다.**
 *
 * S15 가 "거의 전부" 를 숫자로 바꿨다 (2026-08-23, `spike/s15`) — peer 하나가 늘 때:
 *
 * | peer | 우리(`% n`) | consistent 이상값 |
 * |---|---|---|
 * | 3 → 4 | 75.2% | 25.0% |
 * | 7 → 8 | 87.2% | 12.5% |
 * | 16 → 17 | 94.1% | 5.9% |
 *
 * 세션 친화를 쓰는 배포에서 백엔드 하나를 더하면 **거의 모든 세션이 옮겨 간다.**
 * 알고 쓰는 것과 모르고 쓰는 것은 다르므로 숫자를 여기 남긴다.
 *
 * 분포 자체는 문제가 없다 — 흩어진 주소에서 최대 편차 10.1%, 한 /16 서브넷에서 3.7%.
 * 고르는 비용도 무시할 만하다(hash 23.1ns, rr 0.48ns · 요청당 한 번).
 */
